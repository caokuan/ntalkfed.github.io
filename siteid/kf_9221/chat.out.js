...
(function($, undefined) {
    ...
    myScroll.prototype = {
        ...
        _createScroll: function(className) {
            ...
            this.scrollBox = $({
                ...
                style: $.STYLE_NBODY + 'display:block;border-radius:10px;'
            }).appendTo(this.mainBox);
            this.scrollBar = $({
                className: className,
                style: $.STYLE_NBODY + 'background:#d8d8d8;border-radius:10px;position:absolute;width:6px;top:0;'
            }).appendTo(this.scrollBox);
            $({
                tag: 'span',
                style: $.STYLE_NBODY
        ...
        resizeScroll: function() {
            ...
            this.scrollBox.css({
                ...
                left: (mainBoxWidth - _barWidth - _border - 3) + 'px',
                top: '0px'
            });
            this.contentBox.css({
    ...
    chatView.prototype = {
        ...
        _create: function() {
            this.contains = $({
                ...
                key: this.settingid,
                style: $.STYLE_NBODY + 'overflow:hidden;width:100%;height:auto;position:relative;left:0;top:0;padding-top:1px solid #fff\\0;'
            }).appendTo(this.options.chatContainter);

            this.loadElement = $({
                className: 'chat-view-load',
                style: $.STYLE_BODY + 'height:' + this.options.height + 'px;_height:' + (this.options.height - 240) + 'px;box-sizing:border-box;display:block;'
            }).appendTo(this.contains).html(this._getViewHtml('load'));

            this.chatElement = $({
                className: 'chat-view-window',
                style: $.STYLE_BODY + 'width:100%;height:auto;display:none;padding-top:1px solid #fff\\0;'
            }).appendTo(this.contains).html(this._getViewHtml('window'));

            this.messageElement = $({
                className: 'chat-view-message',
                style: $.STYLE_BODY + 'height:' + this.options.height + 'px;display:none;float:left;width:100%;'
            ...
            this.displayiFrame = $({
                ...
                style: $.STYLE_NBODY + 'display:none;'
            ...
            if (!this.options.chatHeader.find('.header-chatrecord-title').length) {
                $({
                    ...
                    style: $.STYLE_BODY + 'font-weight:bold;float:left;margin:15px 10px 5px 20px;height:20px;visibility:visible;overflow:hidden;display:none;color:#ffffff;'
                }).appendTo(this.options.chatHeader.find('.chat-header-body')).html($.lang.button_view);
            }
            if (!this.options.chatHeader.find('.header-chatrecord-close').length) {
                $({
                    className: 'header-chatrecord-close',
                    style: $.STYLE_NBODY + 'float:right;cursor:pointer;margin:20px 5px 0 0;width:20px;height:20px;position:relative;display:none;'
                }).appendTo(this.options.chatHeader);
            }
        ...
        switchUI: function(type) {
            ...
            switch (type) {
                case this.mode.CON_VIEW_WINDOW:
                    ...
                    if (!this.scroll) {
                        this.scroll = new myScroll(this.chatHistory, this.chatHistory.find('ul'), 'chat-view-scrollBar', {
                            width: 411
                        });
                    }
                    break;
                ...
                case "8":
                    ...
                    if (data.from && data.from == 1 && (($.browser.msie && $.browser.ieversion === 9) || broswerAgent.indexOf('firefox') > -1)) {
                        html.push([
                            ...
                            '</video>',
                            '<span class="ntkf-video-button" style="display:block;width:52px;height:52px;background:url(' + $.button + ') center no-repeat;background-size:100%;position:absolute;top:50%;left:0;right:0;margin:-26px auto 0;"></span>'
                        ].join(''));
                        bodyElement.css({
                            'width': '180px',
                            'height': '240px'
                        ...
                        html.push(['<span>', $.lang.cant_play_video, '</span>', ].join(''))
                    } else {
                        html.push([
                        ...
                        if (type == 2) {
                            ...
                            $.require(data.url + '#rnd#image', function(image) {
                                if (image.error) {
                                    ...
                                    //var attr = $.zoom(image, 100, 85);
                                    //设置图片宽高为读取到的宽高
                                    var imageHeight, imageHtml = '<img src="' + data.url + '" sourceurl="' + data.sourceurl + '" width="' + image.width + '" height="' + image.height + '" style="vertical-align:middle;' + $.STYLE_NBODY + 'width:' + image.width + 'px;height:' + image.height + 'px;max-width:220px;max-height:160px;" />';
        ...
        viewLinkContainer: function(data, selector) {
            ...
            $({
                ...
                style: $.STYLE_BODY + 'margin:10px 0 0 0;width:100%;height:24px;white-space:nowrap;text-overflow:ellipsis;-o-text-overflow:ellipsis;overflow:hidden;'
            ...
            $({
                ...
                style: $.STYLE_BODY + 'margin:5px 0 10px 0;width:100%;max-height:60px;overflow:hidden;' + ((nTalk.global.siteid == "kf_9739" || nTalk.global.siteid == "kf_3004") ? 'color:#f00;' : '')
        ...
        suggest: function(data, type) {
            ...
            $.each(data, function(i, message) {
                ...
                }).appendTo(list.find('ul')).html(showMessage).hover(function(event) {
                    $(this).css({
                        'color': '#fff',
        ...
        showInputState: function(position) {
            ...
            $({
                tag: 'span',
                style: 'display:block; left: -24px; top:-20px; position:relative; width:20px; height:20px; background: url(' + $.sourceURI + 'images/mobileicon.png) -110px -70px no-repeat'
        ...
        _showFileUpload: function(liElement, bodyElement, data, type) {
            ...
            if (!success) {
                if (data.error.maxSize) {
                    data.error.error = $.utils.handleLinks($.lang.news_trans_failure_size, {
                        maxsize: data.error.maxSize / (1024 * 1024)
                    });
                } else if (data.error.ext) {
                    data.error.error = $.utils.handleLinks($.lang.news_trans_failure_type, {
                        type: data.error.ext
                    });
                }
            ...
            for (var areaElement, i = 0; i < formOptions.length; i++) {
                if (formOptions[i].type == 'textarea') {
                    ...
                    $({
                        ...
                        style: $.STYLE_BODY + 'font-size:16px;font-weight:bold;color:#ccc;float:right;position:absolute;right:15px;top:70px;'
                    }).appendTo(areaElement).html('0/' + formOptions[i].max / 2);
                }
            }
        ...
        createMessageForm: function(formOptions, disableMessage, announcement, data) {
            ...
            if (data.fileError) {
                ...
                tr = $({
                    tag: 'tr',
                    style: $.STYLE_NBODY
                }).appendTo(this.messageElement.find('.chat-view-message-table tbody'), this.messageElement.find('.chat-view-message-table tbody tr').eq(-1));
                td = $({
                    tag: 'td',
                    style: $.STYLE_NBODY
                }).appendTo(tr);
        ...
        _createfullScreen: function() {
            ...
            if ($('.view-fullScreen-background').length) {
                ...
                $({
                    ...
                    style: $.STYLE_NBODY + 'background:#000;opacity:0.6;filter:alpha(opacity=60);width:' + width + 'px;height:' + height + 'px;position:absolute;top:0;left:0;z-index:2000000000;'
                }).appendTo(true).fixed();
            }
            if ($('.view-fullScreen-container').length) {
                ...
                $({
                    ...
                    style: $.STYLE_NBODY + 'width:' + width + 'px;height:' + height + 'px;text-align:center;position:absolute;top:0px;left:0;z-index:2000000001;'
                }).appendTo(true).html([
        ...
        _getMessageHtml: function(type, data) {
            ...
            return type === 'right' ? [ //发送的消息
                ...
                /left|bottom/gi.test(type) ? [ //接收的消息
                    ...
                    (data.userid && !this.mode.isVisitor(data.userid) && this.mode.dest.id != data.userid && data.systype != (4) ? ['<span class="view-history-destname" style="', $.STYLE_BODY, 'padding-right:5px;float:left;color:#b9b9c1;line-height:26px;">',
                            data.name,
                            '</span>'
                        ].join('') :
                        ''),
                    ...
                    (/^(2|4)$/i.test(data.type) && !data.emotion ? ['<a href="javascript:void(0);" style="', $.STYLE_BODY, '">', $.lang.news_download, '</a>'].join('') : [''].join('')),
                    '</div>',
                    '</div>',
                    '</span>',
                    '</td>',
            ...
            $.each(this.mode.config.faces, function(i, cFace) {
                ...
                if (i === 0) {
                    $({
                        ...
                        title: cFace.name,
                        index: '0',
                        style: $.STYLE_NBODY + 'zoom:1;margin:0 5px 0 0;float:left;background:#fff;position:relative;top:-1px;border-left:1px solid #D4D4D4;border-right:1px solid #D4D4D4;'
                    }).appendTo(self.chatElement.find('.chat-view-face-tags')).append('<img src="' + cFace.icon + '" border="0" style="' + style + 'border:none;" />');
                } else {
                    $({
                        className: 'chat-view-face-tag ' + tagClass,
                        title: cFace.name,
                        index: i,
                        style: $.STYLE_NBODY + 'zoom:1;margin:0 5px 0 0;float:left;position:relative;top:0px;border-left:1px solid #f1f1f1;border-right:1px solid #f1f1f1;'
                    }).appendTo(self.chatElement.find('.chat-view-face-tags')).append('<img src="' + cFace.icon + '" border="0" style="' + style + 'border:none;" />');
                }
            });
            //bind face event
        ...
        _contentFilter: function(data) {
            ...
            if (typeof data.msg !== 'string' || (/<.*?\>/gi.test(data.msg) && data.type != 9) || data.type == 7) {
        ...
        _send: function(isSubmit) {
            ...
            if (!$.browser.html5 && isSubmit === true) {
                this.textEditor.css({
                    'color': '#ccc'
                }).val($.lang.default_textarea_text).get(0).focus();
            }
        },
            ...
            switch (type) {
                case "init":
                    ...
                    var html = ['<div id="duration_', msgid, '" style="', $.STYLE_BODY, 'height:24px;line-height:24px;padding:4px 4px 0px;float:', durationAlign, '" >', duration, '\'\'</div>',
                        ...
                        '<img width="24px" height="24px" src="', pngArs, '"/>', '</div>'
                    ].join("");
    ...
    minimizeView.prototype = {
        ...
        initialize: function(dest, isMessageView, callback) {
            ...
            if (!this.element.length) {
                ...
                this.element = $({
                        ...
                        style: $.STYLE_BODY + 'width:' + (this._width - 2) + 'px;height:' + (this._height - 2) + 'px;border:1px solid #c8c7c6;background:#e5e5e4;cursor:pointer;z-Index:2000000000;'
                    }).appendTo(true)
        ...
        update: function(name, logo) {
            this.title = name ? $.utils.handleLinks($.lang.toolbar_min_title, {
                destname: name
            }) : $.lang.toolbar_default_text;
        ...
        startFlicker: function(highlight, count) {
            ...
            if (highlight) {
                ...
                this.element.css({
                    'border-color': '#c8c7c6'
                }).gradient('top', '#e5e5e4', '#f2f3f3').find('.ntalk-minimize-title').html($.utils.handleLinks($.lang.toolbar_min_news, {
                    count: '<span style="' + $.STYLE_BODY + 'color:#fff;font-weight:bold;">' + messageCount + '</span>'
                }));
            }
            if (count >= 7) return;
    ...
    chatManageView.prototype = {
        ...
        addChatTag: function(settingid) {
            ...
            chatTag = $({
                    tag: 'li',
                    style: $.STYLE_NBODY + 'margin:5px 0 0 5px;list-style:none;border:1px solid #fafafa;border-right:none;position:relative;cursor:pointer;',
        ...
        _create: function() {
            ...
            this._objView = new $.Window($.extend({
                ...
                //页外聊窗居中
            }, options), $('table.containter td.containter-body'));
            ...
            this._objView.buttonMax.display();
            this._objView.buttonMin.display();
            if (this._objView.buttonResize) {
                this._objView.buttonResize.display();
            }
            ...
            this._objView.containter.css({
                'margin': '0 auto',
                'position': 'static'
            });
            this.headBody = $({
                className: 'chat-header-body',
                style: $.STYLE_BODY + 'background:#ebe9e9;z-index:0;color:#525252;'
            }).appendTo(this.header, true).css({
                'position': 'absolute',
                ...
                'height': (this.options.dropHeight - 0) + 'px' //IN
            });

            this.headName = $({
                tag: 'span',
                className: 'chat-header-name',
                style: $.STYLE_BODY + 'color:#ffffff;margin:10px 0px 10px 80px;display:inline-block;float:left;height:20px;line-height:20px;max-width:220px;visibility:hidden;overflow:hidden;font-weight:bold;cursor:auto;font-size:12px;color:#fff;'
            }).appendTo(this.headBody).html('');
            this.headSign = $({
                tag: 'span',
                className: 'chat-header-sign',
                style: $.STYLE_BODY + 'color:#c3c3c3;margin:10px 0px 10px 10px;display:inline-block;float:left;height:20px;line-height:20px;visibility:hidden;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;cursor:auto;'
            }).appendTo(this.headBody);
            this.headIcon = $({
                tag: 'div',
                className: 'chat-header-icon',
                style: $.STYLE_NBODY + 'visibility:hidden;border-radius:0px;overflow:hidden;background:url(' + $.imageicon + ');background-repeat:no-repeat;background-position:-374px 0; background-color:#ffffff;position:absolute;left:20px;top:0px;width:' + this.CON_ICON_WIDTH + 'px;height:' + this.CON_ICON_HEIGHT + 'px;border:1px solid #5f6467;z-index:1;'
            }).appendTo(this.header, true);

            this.chatBody = this._objView.chatBody;

            this.leftElement = $({
                className: 'body-chat-tags',
                style: $.STYLE_NBODY + 'display:none;float:left;background:#fafafa;overflow:hidden;'
            }).css({
                'border-left': '1px solid #5f6467',
            ...
            this.chatContainter = $({
                ...
                style: $.STYLE_NBODY + 'float:left;overflow:hidden;background:#fff;'
            }).css({
                'border-right': '1px solid #5f6467',
                'border-bottom': '1px solid #5f6467',
                'border-left': '1px solid #5f6467',
            ...
            this.rightBody = $({
                ...
                style: $.STYLE_BODY + 'width:199px;background:#fff;'
            }).css({
                'border-right': '1px solid #5f6467',
                'border-bottom': '1px solid #5f6467',
                'height': (+this.options.height - 1) + 'px',
            ...
            //taglist ul
            this.buttonScrollTop = $({
                tag: 'div',
                className: 'nTalk-scroll-top',
                style: $.STYLE_NBODY + 'height:20px;width:100%;z-index:99;background:url(' + $.imageicon + ') no-repeat 20px -92px;display:block;cursor:pointer;'
            }).appendTo(this.leftElement);
            this.leftBody = $({
                tag: 'div',
                className: 'nTalk-scroll-body',
                style: $.STYLE_NBODY + 'overflow:hidden;height:424px;'
            }).appendTo(this.leftElement);
            this.leftContent = $({
                tag: 'ul',
                className: 'ntalke-scroll-content',
                style: $.STYLE_NBODY
            }).appendTo(this.leftBody);
            this.buttonScrollBottom = $({
                tag: 'div',
                className: 'nTalk-scroll-bottom',
                style: $.STYLE_NBODY + 'height:20px;width:100%;z-index:99;background:url(' + $.imageicon + ') no-repeat 20px -108px;display:block;cursor:pointer;'
            }).appendTo(this.leftElement);
        },
        _bind: function() {
            var self = this;
        ...
        _callResize: function(args) {
            ...
            this.rightElement.find('.view-right-content').css({
                'height': rigthConentHeight + 'px'
            });
            this.rightElement.find('.window-right-content iframe').attr('height', rigthConentHeight).css({
                'height': rigthConentHeight + 'px'
            }); //右侧iframe样式、属性同步更新
        ...
        _addRightLabel: function(title, data, length, selected) {
            var self = this,
                expURL = /^https?:\/\/(.*?)/gi,
                key = $.randomChar(),
                listElement, style, tagElement, tagBody,
                rightContentHeight = this.options.height - 28;

            if (!this.rightTags) {
                this.rightTags = $({
                    className: 'window-right-tags',
                    style: $.STYLE_NBODY + 'background:#FAF9F9;z-index:-1;overflow:hidden;height:26px;border-top:2px solid #FFF;'
            ...
            if (!this.rightContent) {
                this.rightContent = $({
                    ...
                    style: $.STYLE_NBODY + 'overflow:hidden;background:#FAF9F9;position:relative;top:-1px;z-index:1;border-radius:0px 0px 5px 0px;-moz-border-radius:0px 0px 5px 0px;-webkit-border-radius:0px 0px 5px 0px'
                }).appendTo(this.rightBody);
            }
            ...
            tagElement = $({
                ...
                title: title,
                style: style
            }).appendTo(this.rightTags, this.rightTags.find('div').last()).html(title); //.gradient("top", '#F8CEDC', '#FAB2CA')
            ...
            if ($.isArray(data)) {
                ...
                listElement = $({
                    tag: 'ul',
                    style: $.STYLE_BODY + 'margin:10px 0 10px 25px;list-style:disc;background-color:#FAF9F9;'
                }).appendTo(tagBody).click(function(event) {
                    var srcElement = $.Event.fixEvent(event).target;
                ...
                for (var i = 0; i < data.length; i++) {
                    $({
                        tag: 'li',
                        talk_id: data[i].id || 'thisisadefaultid',
                        talk_title: data[i].title,
                        talk_content: data[i].con,
                        title: $.clearHtml(data[i].con || ''),
                        style: $.STYLE_BODY + 'list-style:disc outside none;margin-top:5px;cursor:pointer;background-color:#FAF9F9;'
                ...
                $({
                    ...
                    scrolling: 'auto',
                    style: $.STYLE_NBODY + 'width:100%;height:' + rightContentHeight + 'px;'
                }).appendTo(tagBody.css('overflow-y', 'hidden')).attr('src', data);
            } else { //text
                $({
                    className: 'window-right-text',
                    style: $.STYLE_BODY + 'margin:5px;word-wrap:break-word'
                }).appendTo(tagBody).html(data);
            }
            //新创建标签默认选中
            if (selected) {
        ...
        _selectedTag: function(eventElement) {
            ...
            this.rightTags.find('div[class]').each(function(i, elem) {
                ...
                if ($(eventElement).indexOfClass(key)) {
                    ...
                    $(elem).addClass('talk_selected').css({
                        'height': '25px',
                        'border-bottom': 'none',
                        'background-color': "transparent"
                    }); //.gradient('top', '#F8BAC2', '#F9CDDC').css('color', '#DE7E80');
                    self.rightContent.find('.' + key).display(1);
                } else {
                    //$.Log('selected class:' + key + '');
                    $(elem).removeClass('talk_selected').css({
                        'height': '24px',
                        'border-bottom': '1px solid #D5D5D5'
                    }); //.gradient('top', '#F8CEDC', '#FAB2CA').css('color', '#FFFFFF');
        ...
        _addRightTag: function(url, title, closebutton, obj) {
            ...
            if (closebutton == true) {
                ...
                tagContent = '<p style="height:25px;float:left;line-height:25px;text-align:center;white-space:nowrap; overflow:hidden; text-overflow:ellipsis;margin-top:0;">' + title + '</p>'
            } else {
                tagContent = title;
            }
