;(function($, undefined){
	...
	$.chatConnect.prototype = {
		...
		initialize: function(options, close_tchat_flash){
			...
			this.options = $.extend({
				...
				["requestRobot","siteid", "settingid", "tchatmqttserver", "tchatgoserver", "surl", "cid", "u", "n", "sid", "groupid", "rurl", "statictis", "htmlsid","connectid", "userlevel", "disconnecttime", "mini", "chattype", "chatvalue"],
	...
	$.chatMode.prototype = {
		...
		callStat: function(action){
			/**
			 * 4(debug): tchat flash ready
			 * 5(debug): flash初始化完成
			 * 6(debug): flash获取地址成功
			 * 7(debug): flash获取地址失败
			 * 8(debug): flash开始请求分配客服
			 * 9: flash获取分配客服成功
			 * 10:  客服空闲，开始连接客服（含直接进咨询、排队进咨询）（基础统计调用2开启）
			 ...
			 * 24: 访客打开聊窗次数
			 */
			var disableAll = new RegExp("^(0|1|2|4|5|6|7|8|18|19|20|21)", 'gi');
			var disableSection = new RegExp("^(10|11|12|13|22|23)$", 'gi');
			var query = {
				type:		'chatjs',
				siteid:		this.siteid,
                kfid:       this.dest.id || '',
		 ...
		 */
		reconnect: function(element, destid, single){
			if( element ){
		 ...
		 * @return {Boolean}
		 */
		send: function(data, showValue){ //send message
		...
		_filterReceive: function(data){
			...
			data.timestamp	= data.timestamp + this.jetLag;
		...
		initConfig: function(){
			...
			//-------------首条消息，企业Logo、欢迎语-----------------------
			msgContent = !$.browser.mobile&&this.config.logo ? '<p style="' + $.STYLE_BODY + 'background-color:transparent;text-align:center;"><img data-type="ntalk-enterprise-logo" src="'+this.config.logo+'" style="' + $.STYLE_BODY + 'text-align:center;display:inline;" onerror="nTalk.loadImageAbnormal(this, event)" /></p>' : '';
		...
		_initChatConfig: function(){
			...
			$.each(this.config.rightlabel, function(k, item){
				...
				default:
					//自定义页常规参数替换
					item.data = $.utils.handleLinks(item.data, {
						siteid:		self.siteid,
						user_id:	$.global.shortid,
						itemid:		self.itemid || '1111',
						itemparam:	self.options.itemparam
		...
		displayMoreData: function(){
			if( !this.view.displayButton ) return;
	...
	$.chatManage = {
        ...
        htmlSID: "",
        connectId: "",
		open: function(settingid, destid, itemid, itemparam, sellerid, noWaitConnect, single, manual){
			...
			if( !this.hash.contains( this.settingid ) ){
				...
				this.loadConfig(settingid, function(config){
		...
		loadWapView: function (config, callback) {
			...
			if( config.wapTheme && config.wapTheme.layout !== "1" ){
				WapViewFileName = 'chat.view.wap.theme'+config.wapTheme.layout+'.js';
			}
			$.require({
		...
		initChatManage: function(noWaitConnect, config){
			...
			if( config && config.autoconnect == 1 ){
				$.Log('autoconnect:1');
				// 已配置直接连接
				noWaitConnect = true;
		 ...
		 * @return null
		 */
		loadConfig: function(settingid, callback){
			var self = this, chat,
		...
		closeChat: function(settingid){
			...
			this.hash.items( settingid ).close();
		 ...
		 * @return {[type]}
		 */
		showFAQ: function(settingid, title, content){
	...
	$.extend({
		...
		fIM_getSessionCustomerServiceInfo: function(json, settingid){
			var data, chat = $.chatManage.get(settingid);
			if( !chat ){
				return;
			}
	...
	$.extend({
		...
		chatReady: function(){
			...
			$.waitMessage.verificationAdd($.getTime(1), {
				...
				msg: {
					msgtype:	7,
					param:		$.global.erpparam + '|lang=' + ($.global.lang || $.language)
				}
			});
