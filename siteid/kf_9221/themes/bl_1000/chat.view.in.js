;(function($, undefined){
	...
	$.chatView.prototype = {
		...
		_create: function(){
			...
			this._liantong_special();
		},

         //liantong的定制
          _liantong_special:function(){
          
          	var liantong_chat_view_window_bottom=this.contains.find('.chat-view-window-bottom');
            var liantong_chat_view_end_session=this.contains.find('.chat-view-end-session');
            var liantong_chat_view_submit=this.contains.find('.chat-view-submit');
          	var liantong_chat_view_options=this.contains.find('.chat-view-options');
          	var liantong_chat_view_window_history=$('.chat-view-window-history');
          	var liantong_chat_view_window_editor=$('.chat-view-window-editor').find('textarea');
            var liantong_body_chat_containter=$('.body-chat-containter');
            var liantong_ntalk_button_min=$('.ntalk-button-min');
			var liantong_ntalk_button_close=$('.ntalk-button-close');
			var liantong_ntalk_button_maxresize=$('.ntalk-button-maxresize');
            var liantong_chat_view_lbar=$('.chat-view-lbar');
            var liantong_chat_view_options_menu=$('.chat-view-options-menu');
            var liantong_view_fileupload_body=$('.view-fileupload-body');
            var liantong_textarea_placeholder=$('textarea[placeholder]');
			var liantong_ntalk_window_containter=$('.ntalk-window-containter');
			var liantong_ntalk_window_head=$('.ntalk-window-head');
			liantong_ntalk_window_head.css({
				'height':'80px'
			});
			//liantong_ntalk_window_containter.css('border','1px solid #cdcdcd');
            liantong_view_fileupload_body.css({
            	'border-color':'#ffffff'
                
 
            });
        	liantong_textarea_placeholder.css({
        		'color': 'red !important'
        	})
           liantong_chat_view_window_editor.css({
            	height:$.browser.Quirks ? '73px' : '53px',
            	'box-shadow':'white 0px 0px 0px inset'

            });
             liantong_chat_view_window_history.css({
             	'background-color':'#f3f3f3',

             	'width':'453px'
             });
            liantong_body_chat_containter.css({
          	...
          	liantong_chat_view_window_bottom.css({
				'background':'#ffffff',
				'border-bottom': '1px solid #cdcdcd',
				'height': '37px'
			});
          		
			liantong_chat_view_end_session.css({
			margin:'4px 0',
			...
			'line-height': '23px',
          	...
          	liantong_chat_view_submit.css({
                  margin:'4px 8px 0 6px',
                  
                 'background-color':'#448dff',
                  border:'solid 1px #448dff',
                  'font-size':'13px',
                  height:'23px',
                 'line-height': '23px',
                 'border-radius':'4px',
			...
			liantong_ntalk_button_min.css({
				'margin-top':'30px',
				'z-index': '100001'
			});
			liantong_ntalk_button_maxresize.css({
				'margin-top':'30px',
				'z-index': '100001'
			})
			liantong_ntalk_button_close.css({
				'margin-top':'30px',
				'z-index': '100001'
			})
           //发送按钮悬浮在上面
            liantong_chat_view_submit.hover(function(event){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
					'background-color': '#448dff'
				});
				liantong_chat_view_lbar.css('display','none');
			}, function(event){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
					'background-color': '#448dff'
		...
		createEvaluation: function(formOptions, title, startColor, endColor, callback){
			...
			dialogElement.find('input[type!=hidden]').get(0).focus();
			dialogElement.find('textarea').get(0).focus();
					...
					'<table style="',$.STYLE_NBODY,'float:right;_float:none;border-collapse:separate;" class="view-history-right" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'text-align:right;">',
							...
							//2016.03.11更改颜色更改border 顏色值 添加color 值
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#6cbed4;border:1px solid #6cbed4;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">',
								...
								//2016.03.31更改样式增加td//2016.03.18增加头像
								   '<div style="',$.STYLE_NBODY,'margin-right:14px;width:40px; height:40px; position:relative; bottom:0; background:url(',$.imageicon,') no-repeat -376px -278px"></div>',
								'</td>',
							'</tr>',
							'<tr style="',$.STYLE_NBODY,'">',
								'<td style="',$.STYLE_BODY,'overflow:visible;text-align:right;position:relative;">',
								  '<span class="view-chat-hidden-area" style="',$.STYLE_NBODY,'width:1px;height:26px;overflow:visible;position:relative;top:0px;">',
									...
									'</span>',
								//2016.03.18 增加当前客服名
								//2017.05.15 去除发送发送消息是客户名
									/*'<span class="view-history-nowDestname" style="',$.STYLE_BODY,'color:#999999;line-height:26px;font-size:12px;">',
                                    //('\u672a\u767b\u5f55\u9f50\u5bb6\u7f51\u53cb' ||this.mode.user.name ),
                                    ( $.global.uname || '\u672A\u767B\u5F55\u8054\u901A' ),
                                    '</span>',*/
								  //2016.03.11更改颜色
									'<span class="view-history-time" style="',$.STYLE_BODY,'padding-left:6px;color:#999999;line-height:26px;font-size:12px;">',$.formatDate(data.timerkeyid),'</span>',
						...
						'<tbody style="',$.STYLE_NBODY,'">',
							'<tr style="',$.STYLE_NBODY,'">',
							    '<td style="',$.STYLE_NBODY,'width:48px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
									...
									'<div class="view-history-div" style="',$.STYLE_NBODY,'float:left;margin-left:8px; -margin-left:7px; width:40px; height:40px; display:inline-block; position:relative; top:0; background:url(',$.imageicon,') no-repeat -316px -280px">',
							   ...
							   '<td style="',$.STYLE_NBODY,'width:10px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
								...
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#ffffff;border:1px solid #ffffff;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px">',
									...
									(data.type == 1 ? 'color:#' + data.color + ';font:'+(data.italic=="true" ? 'italic' : 'normal')+' '+(data.bold=="true" ? 'bold' : 'normal')+' ' + data.fontsize + 'px/160% Arial,SimSun;color:#333333;text-decoration:' + (data.underline=="true" ? 'underline' : 'none') + ';' : ''),
									'">',
							...
							'<tr style="',$.STYLE_NBODY,'">',
								...
								'<td style="',$.STYLE_BODY,'overflow:visible;position:relative;">',
									...
									//2016.03.18 增加当前客服名
									'<span class="view-history-nowDestname" style="',$.STYLE_BODY,'float:left;color:#999999;line-height:26px;font-size:12px;">',
                                   (data.name || this.mode.dest.name),
									...
									//2016.03.11更改颜色
									'<span class="view-history-time" style="',$.STYLE_BODY,'padding-left:6px;float:left;color:#999999;line-height:26px;font-size:12px;">',
										//客服输入状态消息不显示时间
		...
		_getViewHtml: function(type){
			...
			return type=='load' ?
				...
				[
					...
					'<div class="chat-view-window-toolbar" style="',$.STYLE_BODY,'height:29px;width:100%;background:#ffffff;">',
						//分配客服时状态消息
						...
						'<div class="chat-view-button chat-view-face" title=',$.lang.button_face,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:3px 0 3px 6px;_margin-left:5px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -95px -16px;padding:2px;">',
							'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
						...
						'</div>',
						//2016 更改图标padding 值
						'<div class="chat-view-button chat-view-image" title=',$.lang.button_image,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:4px 0 4px 6px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -118px -16px;padding:2px;"></div>',
						'<div class="chat-view-button chat-view-file" title=',$.lang.button_file,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:4px 0 4px 6px;border:0px solid #ccc;height:17px;display:inline-block;cursor:pointer;width:15px;background:url(',$.imageicon,') no-repeat -140px -16px;padding:2px;"></div>',
						//2016.03.21更改位置
						'<div class="chat-view-button chat-view-capture" title=',$.lang.button_captureImage,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:4px 0 4px 6px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -159px -16px;padding:2px;"></div>',
						/*'<div class="chat-view-capture-options" style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 0px;border:0px solid #ccc;height:16px;display:inline-block;cursor:pointer;">',
							...
							'</div>',
						'</div>',*/
						'<div class="chat-view-button chat-view-evaluate" title=',$.lang.button_evaluation,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:4px 0 4px 6px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -180px -16px;padding: 2px;"></div>',
					...
					'<div class="chat-view-window-editor" style="',$.STYLE_BODY,'height:65px;width:100%;overflow:hidden;">',
						...
						'<textarea placeholder="\u6211\u6709\u95ee\u9898\uff0c\u6211\u8981\u95ee\u002e\u002e\u002e" style="',$.STYLE_BODY,CON_STYLE_SHADOW,'margin:1px;padding:10px 10px 0 10px;width:391px;width:411px\\9;height:73px;height:93px\\9;outline:0px solid #08f;border:0px solid #08f;color:#999999;resize:none;overflow:hidden;"></textarea>',
					'</div>',
					...
					/*	'<div class="chat-view-options" style="',$.STYLE_BODY,'margin:6px 10px 6px 0;float:right;border:1px solid #ccc;width:14px;height:26px;line-height:25px;text-align:center;cursor:pointer;">',
						...
						'<span class="chat-view-end-session" style="',$.STYLE_BODY,'border:1px solid #ebedec;color: #333333;text-decoration:none;margin:4px 10px 8px 0;padding:0 10px;float:right;height:24px;line-height:24px;cursor:pointer;background-color:#f3f3f3;">\u7ed3\u675f\u5bf9\u8bdd</span>',
						//2016.03.22修改颜色
		...
		_bind: function(){
			...
			this.chatElement.find('.chat-view-face').hover(function(event){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
			...
			this.chatElement.find('.chat-view-image').hover(function(e) {
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
			...
			this.chatElement.find('.chat-view-file').hover(function(e){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
			...
			this.chatElement.find('.chat-view-evaluate').hover(function(e){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
			...
			this.chatElement.find('.chat-view-capture').hover(function(e){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
	...
	$.chatManageView.prototype = {
		...
		_create: function(){
			...
			this._objView.buttonClose.hover(function(){
				//$(this).css('background-position', '-60px -20px');
				//2016.03.15
				$(this).css('background-color','#2860D7');
			}, function(){
				//$(this).css('background-position', '-60px 0');
				$(this).css('background-color','');
			...
			this.headBody = $({className: 'chat-header-body', style: $.STYLE_BODY + 'background:#ebe9e9;z-index:0;color:#525252;position: relative;'}).appendTo(this.header, true).css({
				'position': 'absolute',
				...
				'border-bottom': '0',
				'top': '30px',//IN
				'border-right': '1px solid #cdcdcd',//IN
				//'border-radius': '5px 5px 0px 0px',
				//'-moz-border-radius': '5px 5px 0px 0px',
				//'-webkit-border-radius': '5px 5px 0px 0px',
				'width': (this.options.width - 2) + 'px',//IN
				'height': (this.options.dropHeight - 30 ) + 'px',//IN
				'z-index': '10000'
			});
			//2016.03.10 新增logo图片
			
			this.headPic=$({tag: 'img', className:'chat-header-pic', src: $.themesURI + '/logo.png',style: $.STYLE_BODY + 'display:inline-block;height:71px;width:80px;position:absolute;top: -30px;left: 20px;' }).appendTo(this.headBody);
			//2016.03.11更改字体颜色和margin 值
			this.headName = $({tag: 'span', className:'chat-header-name', style: $.STYLE_BODY + 'color:#ebedec;margin:20px 0px 13px 106px;padding-left: 5px;display:inline-block;heigth:12px;font-size:12px;float:left;max-width:220px;border-left:1px solid #ffffff;visibility:hidden;overflow:hidden;cursor:auto;'}).appendTo(this.headBody);
			...
			// this.pptElement         = $({tag: 'iframe', className: 'window-right-pro-images', style: $.STYLE_BODY + 'margin:0px;padding:8px 10px;width:' + (this.options.rightElementWidth-21) + 'px;background:#fff;height:'+ pptElement_tem_height+'px;overflow-x:hidden; scroll-x:none'}).attr({
			// 	//debug temp
			// 	//别人提供的地址
		...
		_callResize: function(args){
			...
			//2017.05.16 更改高度值
			var rigthConentHeight = this.options.height - 44;
			//2016.03.21新添加
		...
		_addRightLabel: function(title, data, length, selected){
			...
			if( !this.rightContent){
				//2016.03.11
				this.rightContent = $({className:'window-right-content', style: $.STYLE_NBODY + 'overflow:hidden;background:#ffffff;position:relative;top:-1px;z-index:1;border-radius:0px 0px 5px 0px;-moz-border-radius:0px 0px 5px 0px;-webkit-border-radius:0px 0px 5px 0px;border-bottom: 1px solid #f3f3f3'}).appendTo(this.rightBody);
			}
			...
			if( $.isArray(data) ){
				...
				$({className: 'window-right-text', style: $.STYLE_BODY + 'margin:5px;color:#666666'}).appendTo(tagBody).html(data);
			}
			//新创建标签默认选中
			if( selected ){
