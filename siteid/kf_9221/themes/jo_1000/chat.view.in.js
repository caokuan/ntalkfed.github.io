;(function($, undefined){
	...
	chatView.prototype = {
		...
		_create: function(){
		 ...
		 //2017.08.02 九牧定制ui
		_jm_special: function(){
			var jm_chat_header_icon = this.options.chatHeader.find('.chat-header-icon');
			var jm_chat_view_window_history = this.contains.find('.chat-view-window-history');
			var jm_chat_view_options = this.contains.find('.chat-view-submit');
			var jm_ntalk_chat_body = this.contains.find('.chat-view-window');
			jm_ntalk_chat_body.css({
				'background-color': '#f6f6f6'
			})
			jm_chat_header_icon.css({
				'border-radius': '50%',
				'-moz-border-radius': '50%',
				'-webkit-border-radius': '50%'
			});
			jm_chat_view_window_history.css({
				'background-color': '#ffffff'
			});
			jm_chat_view_options.css({
				'width': '60px',
				'height': '36px',
				...
				'color': '#fff',
				'line-height': '36px',
				'font-family': 'MicrosoftYaHei,Reguar',
				'border-radius': '5px',
		...
		switchUI: function(type){
			...
			switch(type){
				case this.mode.CON_VIEW_WINDOW:
					...
					if( !this.scroll ){
						this.scroll = new myScroll(this.chatHistory, this.chatHistory.find('ul'), 'chat-view-scrollBar', {width: 692});
					}
					break;
		...
		showMessage: function(position, data){
			...
			style = [
				$.STYLE_NBODY + 'background:transparent;list-style:none outside none;display:block;padding:15px 30px 0 0;',
				$.STYLE_NBODY + 'background:transparent;list-style:none outside none;display:block;padding:15px 0 0 30px;text-align:right;',
				$.STYLE_NBODY + 'background:transparent;list-style:none outside none;display:block;padding:15px 10px 0 10px;text-align:center;'
				];
...
/*			（6.8.2合版）去掉更多按钮
			...
			switch(type+''){
				...
				case "4":
				    if (data.type == 2 && data.emotion == 1) {
				        ...
				        //开始上传图片文件时，显示取消息上传提示
				        position = type == 2 ? '-113px -145px' : '0 -245px';
				        bodyElement.css({
				            'width': '100px',
				            'height': '85px',
				        ...
				        */
				        ...
				        if (type == 2) {
				            ...
				            liElement.find('table').css('width', '138px');
				            //2015.11.10 图片上传失败处理
				            position = type == 2 ? '-27px -145px' : '-98px -245px';
				        ...
				        if (type == 2) {
				            ...
				            $.require(data.url + '#rnd#image', function(image) {
				                ...
				                if (image.error) {
				                    ...
				                    bodyElement.css({
				                        'width': '100px',
				                        'background': 'url(' + $.imageicon + ') no-repeat -27px -145px'
				                    });
					                	}
		...
		_showFileUpload: function(liElement, bodyElement, data, type) {
			...
			//所需图标在大图中的位置
			var statusIconPosition = success ? ' -391px -219px;' : ' -393px -240px;';
			//显示状态名称
		...
		_getMessageHtml: function(type, data){
			...
			if( type === 'otherinfo' ){
				...
				data.msg = [//faq信息
					'<h1 style="',$.STYLE_BODY,'">',
					'<span style="',$.STYLE_NBODY,'float:left;margin-right:5px;width:15px;height:15px;background:transparent url(',$.imageicon,') no-repeat -394px -261px;"></span>',
				...
				[//发送的消息
					'<table style="',$.STYLE_NBODY,'float:right;_float:none;border-collapse:separate;" class="view-history-right" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'text-align:right;">',
							'<tr style="',$.STYLE_NBODY,'">',
							//2016.03.11更改颜色更改border 顏色值 添加color 值
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#3399ff;border:1px solid #3399ff;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">',
									...
									(data.type == 1 ? 'color:#' + data.color + ';font:'+(data.italic=="true" ? 'italic' : 'normal')+' '+(data.bold=="true" ? 'bold' : 'normal')+' ' + data.fontsize + 'px/160% MicrosoftYaHei,Regular;font-size:14px;color:#ffffff;text-decoration:' + (data.underline=="true" ? 'underline' : 'none') + ';' : ''),
									'">',
									(data.type==1 
									? data.msg 
									: data.type==6 
								...
								'<td style="',$.STYLE_NBODY,'width:10px;vertical-align:top;overflow:visible;">',
									...
									//尖角添加className
									'<div class="view-history-angle" style="',$.STYLE_NBODY,'position:relative;left:-3px;top:10px;z-index:1;width:10px;height:18px;background:url(',$.imageicon,') no-repeat -2px -62px;"></div>',//
						...
						'<tbody style="',$.STYLE_NBODY,'">',
							'<tr style="',$.STYLE_NBODY,'">',
							    '<td style="',$.STYLE_NBODY,'width:48px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
									...
									'<div class="view-history-div" style="',$.STYLE_NBODY,'float:left;margin-left:20px; -margin-left:7px; width:40px; height:40px;'+(data.logo?' ':'background:url('+$.imageicon+') no-repeat -325px -47px;')+' display:inline-block; position:relative; top:0;border-radius:50%">',
							   ...
							   '<td style="',$.STYLE_NBODY,'width:10px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
									//2016.03.18更改垂直方向的位置
									//尖角添加className
									'<div class="view-history-angle" style="',$.STYLE_NBODY,'flaot:right;position:relative;top:10px;z-index:1;display:inline-block;width:10px;height:18px;background:url(',$.imageicon,') no-repeat -28px -62px;"></div>',
								'</td>',
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#f6f6f6;border:1px solid #f6f6f6;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px">',
									'<div class="view-history-body" style="min-height:20px;',$.STYLE_BODY,(/^(2|4)$/i.test(data.type)&&!data.emotion ? 'text-align:center;display:table-cell;*display:inline-block;vertical-align:middle;/*width:100px;*/min-height:50px;height:85px;*font-size:0px;*line-height:0px;*font-family:Arial;' : 'display:block;/*width:100%;*/'),'word-break:break-all;word-wrap:break-word;',(type=='bottom' ? 'width:60px;' : ''),
									(data.type == 1 ? 'color:#' + data.color + ';color:#222222;font:'+(data.italic=="true" ? 'italic' : 'normal')+' '+(data.bold=="true" ? 'bold' : 'normal')+' ' + data.fontsize + 'px/160%' + ' ' + 'MicrosoftYaHei,Regular;color:#222222;font-size:14px;text-decoration:' + (data.underline=="true" ? 'underline' : 'none') + ';' : ''),
									'">',
				...
				[
					...
					'<div class="chat-view-window-history" style="',$.STYLE_BODY,'width:100%;height:270px;height:267px\\0;_height:269px;background-repeat:no-repeat;background-position:center bottom; padding-top:1px solid #fff\\0;position:relative;overflow-x:hidden;overflow-y:scroll;">',
						'<ul style="',$.STYLE_NBODY,'list-style:none;margin:10px 0px 10px 0px;">',
							...
							'<div class="chat-view-faq" style="',$.STYLE_BODY,'width:640px;margin: 10px 0 0 20px;padding: 0px 10px 10px 10px;background-color: #f6f6f6;">',
							'</div>',
						'</ul>',
					'</div>',
					'<div class="chat-view-window-toolbar" style="',$.STYLE_BODY,'height:31px;width:100%;border-top:1px solid #cccccc;background:#f6f6f6;">',
						//分配客服时状态消息
						...
						'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
							...
							'</div>',
						'</div>',
						'<div class="chat-view-button chat-view-face" title=',$.lang.button_face,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 20px;_margin-left:5px;border:0px solid #ccc;height:24px;display:inline-block;cursor:pointer;width:24px;background:url(',$.imageicon,') no-repeat -81px 0px;">',
							'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
							...
							'</div>',
						'</div>',
						'<div class="chat-view-button chat-view-image" title=',$.lang.button_image,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 26px;border:0px solid #ccc;height:24px;display:inline-block;cursor:pointer;width:24px;background:url(',$.imageicon,') no-repeat -110px 0px;"></div>',
						'<div class="chat-view-button chat-view-file" title=',$.lang.button_file,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 26px;border:0px solid #ccc;height:24px;display:inline-block;cursor:pointer;width:25px;background:url(',$.imageicon,') no-repeat -138px 0;"></div>',
						'<div class="chat-view-button chat-view-history" title=',$.lang.button_save,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 26px;border:0px solid #ccc;height:24px;display:inline-block;cursor:pointer;width:25px;background:url(',$.imageicon,') no-repeat -167px 0;"></div>',
						//2014.11.11 添加查看聊天记录按钮
						'<div class="chat-view-button chat-view-load-history" title=',$.lang.button_view,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 26px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -220px -40px;"></div>',
						'<div class="chat-view-button chat-view-evaluate" title=',$.lang.button_evaluation,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 26px;border:0px solid #ccc;height:24px;display:inline-block;cursor:pointer;width:24px;background:url(',$.imageicon,') no-repeat -195px 0;"></div>',
						'<div class="chat-view-button chat-view-capture" title=',$.lang.button_captureImage,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0px 26px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -304px -84px;"></div>',
						...
						'<div class="chat-view-switch-manual chat-view-robot-button" title="',$.lang.button_switch_manual,'" style="',$.STYLE_BODY,'color:#525252;float:left;padding:0 0 0 20px;margin:8px 0 0 10px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:auto;background:url(',$.imageicon,') no-repeat -265px -40px;display:none;">',manualText,'</div>',
						'<div class="chat-view-button chat-view-change-csr" title=',$.lang.button_change_csr,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 0 10px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -243px -40px;"></div>',
					...
					'</div>',
					'<div class="chat-view-window-editor" style="',$.STYLE_BODY,'height:76px;width:100%;overflow:hidden;background-color: #f6f6f6">',
						'<textarea placeholder="',$.lang.default_textarea_text,'" style="',$.STYLE_BODY,CON_STYLE_SHADOW,'padding:10px 0 10px 20px;width:381px;width:401px\\9;height:58px;height:78px\\9;color:#222;resize:none;overflow:hidden;overflow-y:auto;"></textarea>',
					'</div>',
					'<div class="chat-view-window-bottom" style="',$.STYLE_BODY,'height:50px;width:100%;background:#f6f6f6;border-radius:0px 0px 5px 5px;-moz-border-radius:0px 0px 5px 5px;-webkit-border-radius:0px 0px 5px 5px">',
						...
						'<span class="chat-view-xiaoneng-version" style="',$.STYLE_BODY,'display:block;visibility:visible;text-decoration:none;margin:12px 0px 12px 20px;float:left;height:26px;line-height:26px;color:#DDD;">',$.lang.chat_xiaoneng_version,'</span>',
      ...
      //keypress计算方式在计算汉字的时候胡出现问题，所以改为keyup计算方式。2016.12.02
			this.textEditor = this.chatElement.find('.chat-view-window-editor textarea').css({
				width: $.browser.Quirks ? '401px' : '381px',
				height:$.browser.Quirks ? '78px' : '58px'
				...
				if( $.browser.msie && $.browser.ieversion<=7 ){
					...
					$(this).css($.merge(css, {
						'width':	($(this).parent().width() - 36) + 'px',
						'height':	($(this).parent().height() - 36) + 'px',
						'border-width': '1px'
					}));
				}else{
				...
				if( $.browser.msie && $.browser.ieversion<=7 ){
					$(this).css({
						"border-width": '0px',
						'width':	($(this).parent().width() - 34) + 'px',
						'height':	($(this).parent().height() - 34) + 'px'
					});
				}else{
					$(this).css({"outline-width": "0"});
				}
				$(this).attr('placeholder',$.lang.default_textarea_text)
				self._stopListen();
			...
			this.chatElement.find('.chat-view-button,.chat-view-switch-manual').hover(function(event){
				...
				positionY = $(this).indexOfClass('chat-view-load-history')||$(this).indexOfClass('chat-view-switch-manual')||$(this).indexOfClass('chat-view-change-csr') ? ' -60px' : ' -27px';
				...
				if( $(this).indexOfClass('chat-view-face') ) {
					...
					positionY = ' -40px';
				}else if($(this).indexOfClass('chat-view-capture')) {
					positionY = ' -84px;'
				}else {
					positionY = ' 0px';
				}
		...
		callChatResize: function(width, height){
			...
			this.chatElement.find('.chat-view-float-history, .chat-view-float-history iframe').css({'height': (height - 168) + 'px'});
	...
	chatManageView.prototype = {
		...
		defaultOptions: {
			...
			width:  700, //聊天窗口区域宽
			height: 550, //聊天窗口区域高
			minWidth: 700,//最小聊天窗口宽
			minHeight:650,//最小聊天窗口高
        ...
        */
		...
		updateRightData: function(settingid, data){
			...
			$.each(data, function(i, obj){
				...
				self._addChatFaq(obj.title, obj.data, data.length, obj.selected);
		...
		_create: function(){
			...
			if ($.themesURI) {
			    ...
			    $.kd_pic_icon= $.themesURI +'pic_icon.jpg';
			   //预加载图片
			...
			this._objView.buttonClose.hover(function(){
				$(this).css('background-position', '-57px -23px ');
			}, function(){
				$(this).css('background-position', '-57px -2px');
			}).attr('title', $.lang.chat_button_close).css({
				margin: '40px 20px 0 0',
				width: '17px',
				height: '17px',
				background: 'url(' + $.imageicon + ') no-repeat -57px -2px'
			});
			...
			this._objView.buttonMax.hover(function(event){
				$(this).css('background-position', '-31px -23px');
			}, function(event){
				$(this).css('background-position', '-31px -2px');
			}).css({
				margin: '40px 14px 0 0',
				width: '17px',
				height: '17px',
				background: 'url(' + $.imageicon + ') no-repeat -31px -2px'
			}).attr('title', $.lang.chat_button_resize_max);

			this._objView.buttonMin.hover(function(){
				$(this).css('background-position', '-4px -23px');
			}, function(){
				$(this).css('background-position', '-4px -2px');
			}).css({
				margin: '40px 14px 0 0',
				width: '17px',
				height: '17px',
				background: 'url(' + $.imageicon + ') no-repeat -4px -2px'
			...
			this.headBody = $({className: 'chat-header-body', style: $.STYLE_BODY + 'background:#ebe9e9;z-index:0;color:#525252;'}).appendTo(this.header, true).css({
				...
				'height': (this.options.dropHeight - 25) + 'px'//IN
			});

			this.headName = $({tag: 'span', className:'chat-header-name', style: $.STYLE_BODY + 'color:#ffffff;margin:15px 0px 15px 75px;display:inline-block;float:left;height:20px;line-height:20px;max-width:220px;visibility:hidden;overflow:hidden;font-weight:bold;cursor:auto;font-size:14px;color:#fff;font-family:MicrosoftYaHei Regular;'}).appendTo(this.headBody).html('');
			this.headSign = $({tag: 'span', className:'chat-header-sign', style: $.STYLE_BODY + 'color:#c3c3c3;margin:15px 0px 15px 10px;display:inline-block;float:left;height:20px;line-height:20px;visibility:hidden;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;cursor:auto;'}).appendTo(this.headBody);
			this.headIcon = $({tag: 'div',  className:'chat-header-icon', style: $.STYLE_NBODY + 'visibility:hidden;border-radius:0px;overflow:hidden;background:url(' + $.imageicon + ');background-repeat:no-repeat;background-position:-374px 0; background-color:#4fadff;position:absolute;left:10px;top:5px;width:' + this.CON_ICON_WIDTH + 'px;height:' + this.CON_ICON_HEIGHT + 'px;z-index:1;'}).appendTo(this.header, true);
		...
		_onOverChatTag: function(event){
			...
			$(target).find('.tag-button-close').css({
				background: 'url('+$.imageicon+') no-repeat -334px -259px'
			});
		...
		/**
		 * @method _addChatFaq   添加常见问题
		 * @param {string} title    标签文本
		 * @param {string} data     常见问题
		 * @param {number} length   标签总数
		 * @param {boolean}selected 是否选中
		 * 2017.08.03
		 */
		 _addChatFaq: function(title, data, length, selected) {
		 	var self = this,
			 listElement,style='display: inline-block;margin:10px 0 0 0;padding-left: 10px;width:200px;height: 40px;line-height: 40px;cursor:pointer;background-color:#ffffff;color: #222222;font-family: MicrosoftYaHei,Reguar;font-size: 14px;overflow:hidden;text-overflow:ellipsis; -o-text-overflow:ellipsis;white-space:nowrap;';
		 	this.chatContains = $('.chat-view-contains');
		 	if(title.indexOf('\u5E38\u89C1\u95EE\u9898') > -1) {
		 		for(var i = 0;i<this.chatContains.length;i++) {
		 			if($(this.chatContains[i]).attr('key') == this.settingid) {
		 				this.chatFaq = $(this.chatContains[i]).find('.chat-view-faq');
		 				//!this.chatFaqTitle
		 				if(!this.chatFaq[0].children.length>0) {
		 					this.chatFaqTitle = $({tag:'div',className:'chat-view-faq-title',style: $.STYLE_NBODY+'width: 640px;height: 40px;color: #222222;font-family: MicrosoftYaHei,Reguar;font-size: 14px;font-weight: bold;line-height: 40px;border-bottom: 1px solid #dddddd;'}).html(title).appendTo(this.chatFaq);
							if($.isArray(data)) {
								//用于常见问题一类的列表形式展示内容
								listElement = $({tag: 'div', style: $.STYLE_BODY + 'background-color:#f6f6f6;'}).appendTo(this.chatFaq).click(function(event){
									var srcElement = $.Event.fixEvent(event).target;
								...
								for(var i = 0; i < data.length; i++){
									if(i == 2 || (i - 2)%3 == 0) {
										$({tag:'span', talk_id : data[i].id || 'thisisadefaultid', talk_title: data[i].title, talk_content: data[i].con, title: $.clearHtml(data[i].con || ''), style: $.STYLE_BODY + style}).appendTo(listElement).html( $.clearHtml(data[i].title || '') );
									} else {
										$({tag:'span', talk_id : data[i].id || 'thisisadefaultid', talk_title: data[i].title, talk_content: data[i].con, title: $.clearHtml(data[i].con || ''), style: $.STYLE_BODY + style+ 'margin-right:5px;'}).appendTo(listElement).html( $.clearHtml(data[i].title || '') );
									}
								}
							}
		 				}
