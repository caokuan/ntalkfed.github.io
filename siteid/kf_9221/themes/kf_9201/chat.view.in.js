	...
	chatView.prototype = {
		...
		initialize: function(options, mode){
			...
			this.buttonSelectors = {
				...
				'exp':     'chat-view-exp',
                'xiaonengver': 'chat-view-inputIng'
			};
		...
		_create: function(){
			...
			this.chatElement    = $({className: 'chat-view-window', style: $.STYLE_BODY + 'width:100%;height:auto;display:none;padding-top:1px solid #fff\\0;border-radius: 0 0 0 3px;'}).appendTo(this.contains).html( this._getViewHtml('window') );
		...
		tounaer: function() {
			var tounaerHeader = $('.ntalk-window-head');
			var tounaerLogo = tounaerHeader.find('.chat-header-icon');
			var tounaerSign = tounaerHeader.find('.chat-header-sign');
			var tounaTalkWindowBody = $('.ntalk-window-body');
			//var tounaTextarea = $('textarea');
			
			tounaerLogo.css('display', 'none');
			tounaerSign.css('display', 'none');
			tounaerHeader.find('.chat-header-name').css({
				'margin': '0 0 0 30px',
				'font-size': '18px',
				'line-height': '50px',
				'font-weight': 'normal',
		...
		showMessage: function(position, data){
			...
			style = [
				$.STYLE_NBODY + 'background:transparent;list-style:none outside none;display:block;padding:10px 77px 0 22px;',
				$.STYLE_NBODY + 'background:transparent;list-style:none outside none;display:block;padding:10px 22px 0 77px;text-align:right;',
				$.STYLE_NBODY + 'background:transparent;list-style:none outside none;display:block;padding:10px 10px 0 10px;text-align:center;'
				];
          ...
          //转接的时候显示离开回话&&加入回话
            if( data.enter && data.enter == 1){
				...
				var inFiveMinute = ((Math.abs(data.timerkeyid-lastArrayTime)<=1000*120) && data.timerkeyid-lastArrayTime>0);
				...
				if( data.type == 1 && position=='left' ){
					//收到消息时，隐藏输入状态
					$('.chat-view-inputIng').css('visibility', 'hidden');
...
/*			（6.8.2合版）去掉更多按钮
				    ...
				    if (data.type == 2 && data.emotion == 1) {
				        ...
				        */
				        ...
				        if (type == 2) {
				            ...
				            liElement.find('table').css('width', '138px');
				            //2015.11.10 图片上传失败处理
				            position = type == 2 ? '-15px -145px' : '-98px -245px';
				        ...
				        if (type == 2) {
				            ...
				            var receiveImage = function(){
				            	$.require(data.url + '#rnd#image', function(image) {
				                    ...
				                    //IE7 兼容处理
				                    liElement.find('table').css('width', ( (width < 220 ? width : 275) + 26 + 35) + 'px');
				                    ...
				                    bodyElement.css({
				                        'background': 'none',
				                        'cursor': 'pointer',
				                        'width': width < 275 ? width + 'px' : '275px',
		...
		showInputState: function(position){
			...
			// }, 5E2);
			$('.chat-view-inputIng').css('visibility', 'visible');
			if($('.chat-view-inputIng').css('visibility')=='visible'){
				clearTimeout(this._inputTimerID);
			    this._inputTimerID = null;
			    this._inputTimerID = setTimeout(function(){
				//2016.03.23
		...
		_showFileUpload: function(liElement, bodyElement, data, type) {
			...
			//IE7下布局调整
			liElement.find('table').css('width', '328px');
		...
		createEvaluation: function(formOptions, title, startColor, endColor, callback){
			var self = this,
				dialogElement,
				html = [
				'<div class="ntkf-alert-close" style="' + $.STYLE_NBODY + 'cursor:pointer;height:50px;position:absolute;right:0px;top:0px;width:34px;background:url(' + $.imageicon + ') no-repeat scroll -65px -23px;-moz-border-radius:0px;-webkit-border-radius:0px;border-radius:0px;"></div>',
				...
				'<td class="chat-view-evaluation-title" colspan="2" style="',$.STYLE_BODY,'text-align:left;height:50px;color:#fff;">',
				'<span style="',$.STYLE_BODY,'color:#000;font-weight:nomal;font-size:14px;vertical-align:middle; margin-left: 30px;">' + '\u611f\u8c22\u60a8\u7684\u652f\u6301\uff0c\u8bf7\u5bf9\u6211\u7684\u670d\u52a1\u8fdb\u884c\u8bc4\u4ef7\uff1a' + '</span>',
				'</td></tr>',
				'<tr>',
					'<td  colspan="2" >',
						'<ul class="star-father" style="' + $.STYLE_BODY + 'width: 150px; float: left; margin-left: 30px;">',
							'<li title="很不满意" style="' + $.STYLE_BODY + 'float: left;list-style: none; cursor:pointer;height:24px;width:24px;background:url(' + $.imageicon + ') no-repeat -100px -0px;" data-star-index="0"></li>',
							'<li title="不满意" style="' + $.STYLE_BODY + 'float: left;list-style: none; cursor:pointer;height:24px;width:24px;background:url(' + $.imageicon + ') no-repeat -100px -0px;" data-star-index="1"></li>',
							'<li title="一般" style="' + $.STYLE_BODY + 'float: left;list-style: none; cursor:pointer;height:24px;width:24px;background:url(' + $.imageicon + ') no-repeat -100px -0px;" data-star-index="2"></li>',
							'<li title="满意" style="' + $.STYLE_BODY + 'float: left;list-style: none; cursor:pointer;height:24px;width:24px;background:url(' + $.imageicon + ') no-repeat -100px -0px;" data-star-index="3"></li>',
							'<li title="很满意" style="' + $.STYLE_BODY + 'float: left;list-style: none; cursor:pointer;height:24px;width:24px;background:url(' + $.imageicon + ') no-repeat -100px -0px;" data-star-index="4"></li>',
							'<br data-star-status="init" style="clear: both;" />',
						'</ul>',
						'<span class="data-star-status" style="' + $.STYLE_BODY + 'float: left;width: 124px; height: 24px; font-size: 14px; font-weight: bold;"></span>',
						'<br data-star-status="init" style="clear: both;" />',
					'</td>',
				'</tr>',
				'<tr>',
					'<td  colspan="2" >',
						'<textarea class="vir-textarea" placeholder="感谢您的反馈, 我们会更加努力." style="' + $.STYLE_BODY + 'padding: 6px 10px 20px 10px;width: 320px; float: left; margin-left: 30px; margin-top: 10px; outline: none; height: 88px; border: 1px solid #cccccc; resize: none;" ></textarea>',
					'</td>',
					
				'</tr>',

				'<tr><table class="ntalker-box-dididi">' ,$.FORM.createInput(formOptions), '</table></tr>',
				'<tr style="',$.STYLE_NBODY,'">',
					'<td colspan="2" style="',$.STYLE_BODY,'text-align:center;">',
						'<input type="button" class="view-alert-submit" value="' + $.lang.evaluation_button_submit + '" style="' + $.STYLE_BODY + 'background:#0c2d5f;height:32px;color:#fff;line-height:32px;display:block;width: 90px;text-align: center; margin: 10px auto 0px;" />',
					'</td></tr>',
					'</tbody>',
				'</table>'
			].join('');
			...
			dialogElement.css({
				...
				'height': '250px',
				'border-radius': '3px',
				'left': parseInt($('.chat-view-contains').css('width')) < 730 ? '65px' : '175px'
			})
			$('.ntkf-alert-close').hover(function() {
				$(this).css({
			...
			for(var areaElement, i=0; i<formOptions.length; i++){
				formOptions[i].optionLine = ''
				if( formOptions[i].type == 'textarea' ){
					areaElement = dialogElement.find('.vir-textarea').parent();
					//2014.11.26 输入字数提示节点需要父级节点为相对定位
					areaElement.css('position', 'relative');
					$({className: 'textarea-' + formOptions[i].name + "-0", maxsize: formOptions[i].max, style: $.STYLE_BODY + 'font-size:16px;font-weight:nomal;color:#ccc;float:right;position:absolute;right:45px;top:100px;'}).appendTo( areaElement ).html('0/' + formOptions[i].max/2);
					areaElements = dialogElement.find('table textarea[name=' + formOptions[i].name + ']').parent();
					areaElements.css('position', 'relative');
					$({className: 'textarea-' + formOptions[i].name, maxsize: formOptions[i].max, style: $.STYLE_BODY + 'font-size:16px;font-weight:nomal;color:#ccc;float:right;position:absolute;right:15px;top:70px;'}).appendTo( areaElements ).html('0/' + formOptions[i].max/2);
				}
			}
			$('.ntalker-box-dididi').css('display', 'none');
			$('.star-father li').bind('mouseenter', function() {
				var index = +$(this).attr('data-star-index');
				if(index < 2){
					$('.star-father li').css('background-position', '-100px -0px');
					for( var i = 0; i <= index; i++) {
						$('.star-father li')[i].style.backgroundPosition ='-123px -0px';
					} 
				} else {
					$('.star-father li').css('background-position', '-100px -0px');
					for( var i = 0; i <= index; i++) {
						$('.star-father li')[i].style.backgroundPosition ='-144px -0px';
					} 	
				}
				...
				if(!$(this).parent().attr('data-selected')){
					...
					var index = +$(this).parent().attr('data-selected');
					if(index < 2){
						$('.star-father li').css('background-position', '-100px -0px');
						for( var i = 0; i <= index; i++) {
							$('.star-father li')[i].style.backgroundPosition ='-123px -0px';
				...
				$(this).parent().attr('data-selected', $(this).attr('data-star-index'));
				self.upateStarLevel();
			});
			...
			dialogElement.find('table textarea').bind('keyup', function(event){
				...
				var selector_0 = 'table .textarea-proposal-0';
				var color_0  = $.enLength($(this).val()) > dialogElement.find(selector_0).attr('maxsize') ? '#f00' : '#ccc';
				var inputText_0= Math.ceil($.enLength($(this).val()) / 2) + '/' + (dialogElement.find(selector_0).attr('maxsize') / 2);
				dialogElement.find(selector_0).html( inputText_0 ).css('color', color_0);
			});
			...
			dialogElement.find('.view-alert-submit').click(function(event){
				$.Event.fixEvent(event).stopPropagation();
				$('.ntalker-box-dididi textarea').val($('.vir-textarea').val());
				//2016.02.14 预防重复点击评价
				...
				if(self.evalRepeatClick && self.evalDialog){
					...
					// self.mode.postEvaluate(formOptions);
				}
		...
		upateStarLevel: function() {
			...
			var num = +$('.star-father').attr('data-selected');
			var cunm = 4-num; 

			$('.data-star-status').html(level[num])
			$('.ntalker-box-dididi input').attr('checked', '');
			$($('.ntalker-box-dididi input')[cunm]).attr('checked', 'true');
			// console.log($('.ntalker-box-dididi input')[cunm]);
		},
		...
		_getMessageHtml: function(type, data){
			...
			var contentWidth = $('.chat-view-window-history').find('ul').width();
			...
			if( type === 'otherinfo' ){
				...
				data.msg = [//faq信息
					'<h1 style="',$.STYLE_BODY,'">',
					'<span style="',$.STYLE_NBODY,'float:left;margin-top:3px;margin-right:7px;width:15px;height:15px;background:transparent url(',$.imageicon,') no-repeat -199px -40px;"></span>',
					'<span style="',$.STYLE_BODY,'">',data.title,'</span>',
			...
			return type === 'right' ?
				[//发送的消息
					(data.showTime ? '' : ['<span style="', $.STYLE_BODY, ';color:#bcbcbc;display: block;width: 100%;text-align: center;margin-top: 5px;margin-bottom: 10px;"><span class="view-history-time" style="', $.STYLE_BODY, ';color:#bcbcbc;display: inline-block; background: #ebebeb; min-width: 48px; height: 24px; font-size:12px;line-height:24px; text-align: center;border-radius: 3px;">',
				    this.formatMessageTime(data.timerkeyid),
				    // console.log(data.timerkeyid)
				    '</span></span>'
				].join('')),
					'<table style="',$.STYLE_NBODY,'float:right;_float:none;border-collapse:separate;" class="view-history-right" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'text-align:right;">',
							'<tr style="',$.STYLE_NBODY,'margin-right: 55px;">',
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#c2daff;border:1px solid #98bbf0;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;position: relative;">',
									...
									'</div>',
									//尖角添加className
									'<div class="view-history-angle" style="',$.STYLE_NBODY,'position:absolute;right:-15px;top:5px;z-index:1;width:15px;height:18px;background:url(',$.themesURI,'chaticon.png) no-repeat -11px -58px;margin:0;"></div>',
								'</td>',
								'<td style="',$.STYLE_NBODY,'width:55px;height:100%;position:relative;">',
									...
									'<div style="',$.STYLE_NBODY,'width:55px;height:40px;float:right;position:absolute;top: 0px;right:0px">',
										'<div style="'+$.STYLE_NBODY+'width:40px;height:40px;float:right;background:url('+$.themesURI+'chaticon.png) no-repeat -334px -62px;"></div>',
										//'<div class="view-history-angle" style="',$.STYLE_NBODY,'position:absolute;left:-1px;top:5px;z-index:1;width:15px;height:18px;background:url(',$.themesURI,'chaticon.png) no-repeat -11px -58px;margin:0;"></div>',//
									'</div>',
								'</td>',
							'</tr>',
							// '<tr style="',$.STYLE_NBODY,'">',
				...
				[//接收的消息
				(data.showTime ? '' : ['<span style="', $.STYLE_BODY, ';color:#bcbcbc;display: block;width: 100%;text-align: center;margin-top: 5px;margin-bottom: 10px;"><span class="view-history-time" style="', $.STYLE_BODY, ';color:#bcbcbc;display: inline-block; background: #ebebeb; min-width: 48px; height: 24px; font-size:12px;line-height:24px; text-align: center;border-radius: 3px;margin-left:110px;">',
				    this.formatMessageTime(data.timerkeyid),
				    // console.log(data.timerkeyid)
				    '</span></span>'
				].join('')),
					'<table style="',$.STYLE_NBODY,'float:left;float:none;table-layout:auto;border-collapse:separate;" class="view-history-left" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'">',
							'<tr>',
								'<td style="',$.STYLE_NBODY,'width:52px;height: 100%;vertical-align:middle;overflow:visible;position:relative;" rowspan="2">',
									// add className for angle
 									'<div style="',$.STYLE_NBODY,'width:52px;height:40px;position:absolute;top:0;left:0px;">',
										...
										:
											...
											'<div style="'+$.STYLE_BODY+'width:40px;height:40px;border-radius:40px;-moz-border-radius:40px;-webkit-border-radius:40px;float:left;display:block;text-align:left;margin-right:3px;background:url('+$.themesURI+'chaticon.png) no-repeat -384px -62px;"></div>'),
										'<div class="view-history-angle" style="',$.STYLE_NBODY,'position:absolute;right:-2px;top:32px;z-index:1;width:20px;height:18px;background:url(',$.imageicon,') no-repeat 10px -59px;"></div>',
									'</div>',
								'</td>',
								'<td style="',$.STYLE_BODY,'padding-bottom: 5px;">',(data.name || this.mode.dest.name),'</td>',
							'</tr>',
							'<tr style="',$.STYLE_NBODY,'">',

								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#ffffff;border:1px solid #e6e6e6;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px">',
			...
			return type=='load' ?
				...
				[
					...
					'<div class="chat-view-float-history" style="',$.STYLE_BODY,'width:100%;height:270px;height:267px\\0;_height:269px;background:#fff;padding-top:1px solid #fff\\0;position:absolute;overflow:hidden;z-index:99;display:none;box-shadow:0 5px 3px #888888;">',
						...
						'</iframe>',
					'</div>',
					'<div class="chat-view-window-history" style="',$.STYLE_BODY,'width:100%;height:350px;height:267px\\0;_height:269px;background-repeat:no-repeat;background-position:center bottom; padding-top:1px solid #fff\\0;position:relative;overflow-x:hidden;overflow-y:scroll;background-color: #f1f3f6">',
						'<ul style="',$.STYLE_NBODY,'list-style:none;margin:10px 0px 10px 0px;">',
							//'<li style="',$.STYLE_NBODY,'list-style:none;"></li>',
						'</ul>',
					'</div>',
					'<div class="chat-view-window-toolbar" style="',$.STYLE_BODY,'height:40px;width:100%;border-top:1px solid #e5e5e5;background:#ffffff;">',
						//分配客服时状态消息
						...
						//2015.01.05 添加输入提示建议
						'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
							'<div class="chat-view-suggest-list chat-view-span" style="',$.STYLE_NBODY,'border:1px solid #999;background:#fafafa;width:506px;line-height:30px;height:auto;position:absolute;top:-2px;left:2px;z-index:999;display:none;">',
								'<ul style="',$.STYLE_BODY,'list-style:none;"></ul>',
							'</div>',
						'</div>',
						'<div class="chat-view-button chat-view-face" title=',tounaerTit.face,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:5px 0 5px 20px;_margin-left:5px;border:0px solid #ccc;height:30px;display:inline-block;cursor:pointer;width:36px;background:url(',$.imageicon,') no-repeat -285px -214px;">',
							'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
								'<div class="chat-view-span chat-view-window-face" style="',$.STYLE_NBODY,'display:none;position:absolute;left:-11px;top:-220px;width:247px;height:214px;background:#fff;z-index:1000002;cursor:auto;border-radius:3px;overflow:hidden;box-shadow:0 0 10px rgba(0,0,0,0.9)">',

								'</div>',
							'</div>',
						'</div>',
						'<div class="chat-view-button chat-view-image" title=',tounaerTit.pic,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:5px 0 5px 0px;border:0px solid #ccc;height:30px;display:inline-block;cursor:pointer;width:36px;background:url(',$.imageicon,') no-repeat -312px -214px;"></div>',
						'<div class="chat-view-button chat-view-file" title=',$.lang.button_file,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:5px 0 5px 0px;border:0px solid #ccc;height:30px;display:inline-block;cursor:pointer;width:36px;background:url(',$.imageicon,') no-repeat -340px -214px;"></div>',
						'<div class="chat-view-button chat-view-history" title=',tounaerTit.download,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:5px 0 5px 0px;border:0px solid #ccc;height:30px;display:inline-block;cursor:pointer;width:36px;background:url(',$.imageicon,') no-repeat -368px -214px;"></div>',
						...
						'<div class="chat-view-button chat-view-capture" title=',tounaerTit.cap,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:5px 0 5px 0px;border:0px solid #ccc;height:30px;display:inline-block;cursor:pointer;width:32px;background:url(',$.imageicon,') no-repeat -394px -214px;"></div>',
						'<div class="chat-view-capture-options" style="',$.STYLE_BODY,'color:#525252;float:left;margin:10px 0;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;">',
							'▼',
						...
						'<div class="chat-view-switch-manual chat-view-robot-button" title="',$.lang.button_switch_manual,'" style="',$.STYLE_BODY,'color:#525252;float:left;width: 36px; height: 20px;margin:8px 7px;border:0px solid #ccc;display:inline-block;cursor:pointer;background:url(',$.imageicon,') no-repeat -265px -40px;display:none;">',manualText,'</div>',
						...
						'<div class="chat-view-button chat-view-exp" style="',$.STYLE_BODY,'color:#525252;float:left;margin:0;padding:0 3px;border:0px solid #ccc;height:1px;width: 1px;display:none;visibility: hidden;cursor:pointer;">',$.lang.button_more,' &gt;</div>',
						'<span class="chat-view-button chat-view-evaluate" title="\u60a8\u8fd8\u6ca1\u5f00\u59cb\u5bf9\u8bdd" style="',$.STYLE_BODY,'color:#666;float:left;height:30px;line-height: 24px !important; margin: 5px 0;cursor:pointer;width:80px;background:url(',$.imageicon,') no-repeat -373px -272px; float: right; text-indent: 25px">\u8bc4\u4ef7</span>',
					'</div>',
					'<div class="chat-view-window-editor" style="',$.STYLE_BODY,'height:60px;width:100%;overflow:hidden;padding: 0 0 0 30px;-moz-box-sizing: border-box;-webkit-box-sizing: border-box;-o-box-sizing: border-box;-ms-box-sizing: border-box;box-sizing: border-box; ">',
						'<textarea placeholder="',$.lang.default_textarea_text,'" style="',$.STYLE_BODY,'height:60px;outline:none;border:0px solid #08f;color:#ccc;resize:none;overflow:hidden;overflow-y:auto;width: 100%;scrollbar-arrow-color:yellow;scrollbar-base-color:lightsalmon;"></textarea>',
					'</div>',
					'<div class="chat-view-window-bottom" style="',$.STYLE_BODY,'height:59px;width:100%;background:#ffffff;border-radius:0px 0px 0px 3px;-moz-border-radius:0px 0px 0px 3px;-webkit-border-radius:0px 0px 0px 3px">',
						'<div class="chat-view-options" style="',$.STYLE_BODY,'margin:12px 30px 0 0;float:right;width:30px;height:32px;line-height:32px;text-align:center;cursor:pointer;background: #0c2d5f;color: #fff;">',
							//'▼',
							'<div style="',$.STYLE_BODY,'width:20px;height:20px;background:url('+$.themesURI+'chaticon.png) no-repeat -77px -105px;margin: 6px 5px"></div>',
							'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:1px;height:1px;position:relative;overflow:visible;">',
								'<div class="chat-view-span chat-view-options-menu" style="',$.STYLE_BODY,'display:none;padding:1px;background:#fff;position:absolute;left:-113px;top:-95px;border:1px solid #5e5e5e;width:139px;*width:102px;_width:102px;height:auto;z-index:1000006;cursor:cursor;">',
									//发送消息方式
									'<div class="view-option-enter talk_selected" style="',$.STYLE_BODY,'padding:3px 0 3px 24px;height:24px;line-height: 24px;font-size: 14px;color:#666;position: relative;">',
									'<span style="',$.STYLE_BODY,'width: 24px;height:24px;line-height: 24px;text-align: center;font-size: 14px;display: inline-block;position: absolute;top:3px;left:0px;">\u2714</span>','\u6309',$.lang.button_enter,'\u53d1\u9001','</div>',
									'<div class="view-option-ctrl+enter" style="',$.STYLE_BODY,'padding:3px 0 3px 24px;height:24px;line-height: 24px;font-size: 14px;color:#666;position: relative;">',
									'<span style="',$.STYLE_BODY,'width: 24px;height:24px;line-height: 24px;text-align: center;font-size: 14px;display: none;position: absolute;top:3px;left:0px;">\u2714</span>','\u6309',$.lang.button_ctrl_enter,'\u53d1\u9001','</div>',
								'</div>',
							'</div>',
						'</div>',
						'<div class="chat-view-shu" style="',$.STYLE_BODY,'margin-top:12px;float:right;width:1px;height:32px; background:url(',$.imageicon,') no-repeat -373px 0px;"></div>',
						'<div class="chat-view-submit" style="',$.STYLE_BODY,'margin-top:12px;float:right;width:60px;height:32px;line-height:32px;text-align:center;cursor:pointer;color: #fff; background: #0c2d5f;">',$.lang.chat_button_send,'</div>',
						'<span class="chat-view-end-session" style="',$.STYLE_BODY,'text-decoration:none;margin:16px 25px 8px 0;padding:0 10px;float:right;height:24px;line-height:24px;cursor:pointer;font-size: 14px; color: #666;">',$.lang.button_end_session,'</span>',
						//'<span class="chat-view-xiaoneng-version" style="',$.STYLE_BODY,'display:block;visibility:visible;text-decoration:none;margin:16px 0px 0px 30px;float:left;height:26px;line-height:26px;color:#DDD;">',$.lang.chat_xiaoneng_version,'</span>',
      ...
      //keypress计算方式在计算汉字的时候胡出现问题，所以改为keyup计算方式。2016.12.02
        ...
        self._editorStart = self._getPositionForTextArea(this) + 1;
			...
			this.chatElement.find('.chat-view-button,.chat-view-switch-manual').hover(function(event){
				...
				positionY = $(this).indexOfClass('chat-view-load-history')||$(this).indexOfClass('chat-view-switch-manual')||$(this).indexOfClass('chat-view-change-csr') ? ' -60px' : ' -242px';
				if( $(this).indexOfClass('chat-view-evaluate') ) {
					if($(this).attr('talk_disable') == 'disable') {
						$(this).attr('title', '\u60a8\u8fd8\u6ca1\u5f00\u59cb\u5bf9\u8bdd')
					} else {
						$(this).attr('title', '\u5ba2\u670d\u8bc4\u4ef7')
					}
					positionY = ' -300px'
					 $(this).css('color', '#53c0ff')
				}
			...
			this.chatElement.find('.chat-view-options').hover(function(event){
				$.Event.fixEvent(event).stopPropagation();
				$(this).css({
					'background-color': '#53c0ff'
				});
		...
		_initFaceGroup: function(){
			...
			if( !this.chatElement.find('.chat-view-face-tags').length ){
				this.chatElement.find('.chat-view-window-face').append(['<div class="chat-view-face-tags" style="',$.STYLE_NBODY,'background:#F1F1F1;clear:both;margin-top: 10px;height:38px;border-top:1px solid #D4D4D4;"></div>'].join(''));
			}
			//init face group
			$.each(this.mode.config.faces, function(i, cFace){
				...
				if( !self.chatElement.find('.' + groupClass).length ){
					self.chatElement.find('.chat-view-window-face').insert('<div class="' + groupClass + ' chat-view-face-group" style="' + $.STYLE_NBODY + (i === 0 ? '' : 'display:none;') + 'overflow-x:hidden;overflow-y:auto;clear:left;height:165px;"></div>', 'afterbegin');
				}
				...
				//组标签
				if( i === 0 ){
					$({className: 'chat-view-face-tag ' + tagClass + ' tag-selected', title: cFace.name, index: '0', style:$.STYLE_NBODY + 'zoom:1;margin:0 5px 0 0;float:left;background:#fff;position:relative;top:-1px;'}).appendTo(self.chatElement.find('.chat-view-face-tags')).append('<img src="' + cFace.icon + '" border="0" style="' + style + 'border:none;" />');
				}else{
					$({className: 'chat-view-face-tag ' + tagClass, title: cFace.name, index: i, style:$.STYLE_NBODY + 'zoom:1;margin:0 5px 0 0;float:left;position:relative;top:0px;'}).appendTo(self.chatElement.find('.chat-view-face-tags')).append('<img src="' + cFace.icon + '" border="0" style="' + style + 'border:none;" />');
				}
			});
			//bind face event
		...
		callChatResize: function(width, height){
			...
			this.chatHistory.css({'height': (height - 160) + 'px'});
	...
	chatManageView.prototype = {
		...
		defaultOptions: {
			...
			width:  520, //聊天窗口区域宽
			height: 510, //聊天窗口区域高
			minWidth: 520,//最小聊天窗口宽
			minHeight:610,//最小聊天窗口高
		...
		updateChatTag: function(settingid, data, updateStatus){
			...
			if( !data.id ){
				this.header.find(".chat-header-icon,.chat-header-sign").css('visibility', 'hidden');
				return;
			}
			...
			icon.css('visibility', 'visible').css('background-image', 'none').css('display','none');
		...
		_create: function(){
			...
			var timeoutTime = setTimeout(function() {
				$.require($.protocolFilter($.themesURI + 'iscroll.js'), function() {
					var iscroll = new IScroll('#listScroll',{
						scrollbars: true,
						mouseWheel: true,
					...
					intervalTime = setInterval(function () { 
						var resultContentH = $("#enterpriseScroll").height(); 
						if (resultContentH > 0) {
							//判断数据加载完成的条件 
							$("#enterpriseScroll").height(resultContentH); 
							setTimeout(function () {
								clearInterval(intervalTime); 
								var iscroll1 = new IScroll('#enterpriseScroll',{
									scrollbars: true,
									mouseWheel: true,
									interactiveScrollbars: true,
									shrinkScrollbars: 'scale'
			...
			this.chatWidth = this.options.width;
			this.btnHover = $({tag:'div',title:'收起',className:'nTalk-btnHover',style:'width:30px;height:350px;position:absolute;top:50px;left:504px;z-index:15;display:block;cursor:pointer;background: parseparent'}).appendTo($('.ntalk-window-containter'));
			this.btnHover_arrow=$({tag:'a',className:'nTalk-btnHover-arrow',style:'display:none;width:16px;height:62px;position:absolute;top:195px;left:0px;background:url('+$.themesURI+'chaticon.png) no-repeat -326px -269px;z-index:5;background-color:transparent;'}).appendTo(this.btnHover);
			...
			this._objView.buttonClose.hover(function(){
				$(this).css('background-color', '#ff823e');
			}, function(){
				$(this).css('background-color', '');
			}).attr('title', $.lang.chat_button_close).css({
				margin: '0',
				background: 'url(' + $.imageicon + ') no-repeat -64px 14px'
			}).css({
				width: "34px",
				height: "50px"
			});
			...
			this._objView.buttonMax.hover(function(){
				$(this).css('background-color', '#30507f');
			}, function(){
				$(this).css('background-color', '');
			}).css({
				margin: '0',
				background: 'url(' + $.imageicon + ') no-repeat -31px 14px'
			}).attr('title', $.lang.chat_button_resize_max).css({
				width: "34px",
				height: "50px"
			});
			...
			this._objView.buttonMin.hover(function(){
				...
				margin: '0',
				background: 'url(' + $.imageicon + ') no-repeat 2px 14px'
			}).attr('title', $.lang.chat_button_min).css({
				width: "34px",
				height: "50px"
			});
			...
			this.inputIng = $({tag: 'span', className: 'chat-view-inputIng', style: "',$.STYLE_BODY,'display:block;visibility:visible;text-decoration:none;margin-left:16px;float:left;height:50px;line-height:55px;color:#fff;"})
			                .html('\u5bf9\u65b9\u6b63\u5728\u8f93\u5165\u4fe1\u606f\u002e\u002e\u002e').appendTo(this.headBody);			 
            ...
            */
			...
			this.rightBody = $({className: 'window-right-body', style: $.STYLE_BODY + 'height: 309px; width:219px;background:#fff;'}).css({
				'border-right':  '1px solid #dbdbdb',
				overflow: 'hidden'
			}).appendTo(this.rightElement);
			// console.log(this.options.height);
			this.rightExt = $({className: 'window-right-ext',  style: $.STYLE_BODY + 'width:219px;background:#fff;'}).css({
				'height': '200px',
				'border-bottom': '1px solid #dbdbdb',
				'border-right':  '1px solid #dbdbdb',
			...
			$({ tag: 'iframe',
				src: 'https://www.touna.cn/user-erp-selfservice.html',
				frameBorder: 0,
				width: '220px',
				height: '200px'
			}).appendTo(this.rightExt)
		...
		_bind: function(){
			...
			this.btnHover.hover(function(){
				...
				if($(this).attr('title') == '展开') {
					self.toggleRightLabel(1);
					$('.nTalk-btnHover').attr('title','收起');
					$('.chat-view-window-status-info').css('width','740px');
				} else {
					self.toggleRightLabel(0);
					$('.chat-view-window-status-info').css('width','520px');
		...
		_callResize: function(args){
			...
			// this.body.css('width', ( this.options.width + (this.extended.leftElement ? this.options.leftElementWidth : 0) ) + 'px');
				...
				// 	height:(this.options.height - 1) + "px"
				// });
			this.chatContainter.css('height',(this.options.height - 1)+'px');
			...
			//$('.ntalk-window-right').css('height',(this.options.height - 1)+'px');
			//侧边栏内容区高，去除侧边栏标签区高
			...
			var rigthConentHeight = this.options.height - 32 -200;
			this.rightElement.find('.window-right-body').css('height',(rigthConentHeight+32)+'px');
		...
		_callMaxResize: function(){
			...
			this._objView.changeAttr(this.chatWidth, this.chatHeight);
			if( setMax ){
				this._objView.buttonMax.css('background-position', '-30px 14px').attr('title', $.lang.chat_button_resize_min);
			}else{
				this._objView.buttonMax.css('background-position', '-30px 14px').attr('title', $.lang.chat_button_resize_max);
			}
		...
		_addRightLabel: function(title, data, length, selected){
			var self = this,
				expURL = /^https?:\/\/(.*?)/gi,
				key = $.randomChar(), listElement, style, tagElement, tagBody,enterprise,scroll,
				rightContentHeight = this.options.height - 32 -200;
				// console.log(rightContentHeight);
			if(title.indexOf('\u5173\u4E8E\u4F01\u4E1A') > -1) {
				title = '\u4F01\u4E1A\u5E73\u53F0';
			}
			if( !this.rightTags ){
				this.rightTags = $({className:'window-right-tags', style: $.STYLE_NBODY + 'background:#FAF9F9;z-index:-1;overflow:hidden;height:32px;'}).appendTo(this.rightBody);
			...
			style = $.STYLE_BODY + 'background-color:#f1f3f6;height:32px;line-height:32px;text-align:center;cursor:pointer;float:left;';

			if( length == 1 ){
				style += 'width:219px;';
			}else{
				if( this.rightTags.find('div').length == 1 ){
					style += 'width:' + (length == 2 ? 110 : 72) + 'px;';
				}else if( this.rightTags.find('div').length < length ){
					style += 'width:' + (length == 2 ? 109 : 72) + 'px;';//border-left:1px solid #FCE4E7;
				}else{
					style += 'width:' + (length == 2 ? 109 : 73) + 'px;';//border-left:1px solid #FCE4E7;
				}
			}

			tagElement = $({className: key, title: title, style: style}).appendTo(this.rightTags, this.rightTags.find('div').last()).html(  '<span style="width: 56px; height: 30px; display: block; margin: 0 auto;font-size: 14px;"> '+ title +'</span>');//.gradient("top", '#F8CEDC', '#FAB2CA')
			tagBody = this.rightContent.insert( ['<div class="',key,' view-right-content"  style="',$.STYLE_BODY,'background-color:#fff;width:219px;position: relative;width:219px;height:' + rightContentHeight + 'px;overflow-x:hidden;overflow-y:hidden;display:none;border-radius:0px 0px 5px 0px;-moz-border-radius:0px 0px 5px 0px;-webkit-border-radius:0px 0px 5px 0px;"></div>'].join('') );
			
			if( $.isArray(data) ){
				//用于常见问题一类的列表形式展示内容
				listScroll = $({tag: 'div',id: 'listScroll',style: 'margin: 0;padding: 0;width: 219px;height: 100%;'}).appendTo(tagBody);
				listElement = $({tag: 'ul',className:'scroll-right', style: $.STYLE_BODY + 'margin:12px 0 0 16px;list-style:disc;background-color:#fff;'}).appendTo(listScroll).click(function(event){
					var srcElement = $.Event.fixEvent(event).target;
				...
				for(var i = 0; i < data.length; i++){
					$({tag:'li', talk_id : data[i].id || 'thisisadefaultid', talk_title: data[i].title, talk_content: data[i].con, title: $.clearHtml(data[i].title || ''), style: $.STYLE_BODY + 'position: relatlist-style:disc outside none;cursor:pointer;background-color:#fff;list-style: none; height: 26px; line-height: 26px; color: #666;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'}).appendTo(listElement).html( $.clearHtml(data[i].title || '') ).hover(function(){
						$(this).css({
							'color': '#53c0ff'
						})
					},function() {
			...
			}else if(title.indexOf('\u4F01\u4E1A\u5E73\u53F0') > -1) {
				enterpriseScroll = $({tag: 'div',id: 'enterpriseScroll',style: 'margin: 0;padding: 0 15px;width: 189px;height: 100%;'}).appendTo(tagBody);
				enterprise = $({tag: 'ul', className:'scroll-right', style: $.STYLE_BODY + 'margin:0;list-style:none;background-color:#fff;'}).appendTo(enterpriseScroll);
				$({tag:'li', style: $.STYLE_BODY + 'position: relatlist-style:disc outside none;background-color:#fff;list-style: none;'}).html('<img src="'+$.themesURI+'QRCode.png" style="'+$.STYLE_BODY+'width: 110px;height: 110px;margin-top:10px;margin-left: 39px;"/>').appendTo(enterprise);
				//$({tag:'img',style: $.STYLE_BODY+'width: 110px;height: 110px;margin-top:10px;margin-left: 40px;',src: $.themesURI+'QRCode.png'}).appendTo(enterprise);
				var arr = ['投哪网：2012年5月4日成立，注册资本1亿，是国内领先的互联网理财平台。',
							'运营模式：投哪网为有资金需求的借款者和有闲散资金的投资者搭建了诚信、规范、专业、透明的互联网金融平台。',
							'风控：小额、分散、抵押、直营',
							'股东背景：:广发证券(券商)；大金重工(上市)；中银粤财（国资）。',
							'行业地位：',
							'中国互联网金融协会首批会员单位、',
							'广东互联网金融协会副会长单位、',
							'深圳互联网金融协会副会长单位']
				for(var i=0; i < arr.length; i++) {
					$({tag:'li', style: $.STYLE_BODY + 'margin:0;width: 100%;list-style:none;background-color:#fff;line-height: 16px;'}).appendTo(enterprise).html(arr[i]);
				}
			}else{//text
		...
		_selectedTag: function(eventElement){
			...
			this.rightTags.find('div[class]').each(function(i, elem){
				...
				if( $(eventElement).indexOfClass( key ) ){
					...
					$(elem).addClass('talk_selected').css({'height': '32px', 'border-bottom': 'none','background-color':"#fff", 'color': '#0c2d5f'});//.gradient('top', '#F8BAC2', '#F9CDDC').css('color', '#DE7E80');
					$(elem).find('span').css('border-bottom', '1px solid #0c2d5f');
					self.rightContent.find('.'+key).display(1);
				}else{
					//$.Log('selected class:' + key + '');
					$(elem).removeClass('talk_selected').css({'height': '32px', 'border-bottom': 'none','color': '#666666', 'background-color':"#f1f3f6"});//.gradient('top', '#F8CEDC', '#FAB2CA').css('color', '#FFFFFF');
					$(elem).find('span').css('border-bottom', 'none');
        ...
        */
        _addRightTag:function(url,title,closebutton,obj){
            ...
            var rightContentHeight = this.options.height - 32;
            var self = this;
        ...
        */
		...
		//  投哪兒網 ui 定製 右側動畫
		toggleRightLabel: function(arrow) {
			var self = this, speed, targetWidth,
			currentWidth = parseInt($('.ntalk-window-body').css('width'));
			$('.ntalk-window-right').css('display', 'block');
			...
			this.timer = setInterval(function moveRightTags() {
				currentWidth = parseInt($('.ntalk-window-body').css('width'))// parseFloat($('.ntalk-window-body').css('width')) > parseInt($('.ntalk-window-body').css('width')) ? parseInt($('.ntalk-window-body').css('width')) + 1 : parseInt($('.ntalk-window-body').css('width'));

				speed = !arrow ? 10 : -10; //Math.abs((targetWidth - currentWidth))/8 < 1 * (targetWidth - currentWidth) / Math.abs((targetWidth - currentWidth))  ? 1 : (targetWidth - currentWidth)/8;

				$('.nTalk-btnHover').css('display', 'none');

				if (arrow && currentWidth <= targetWidth ) {
					clearInterval(self.timer);
					$('.nTalk-btnHover').css('left', '490px');
					$('.nTalk-btnHover').css('display', 'block');

					$('.nTalk-btnHover-arrow').css({'background-position':'-326px -270px', 'left': '16px'});
				} else if(!arrow &&  currentWidth >= targetWidth ) {
					clearInterval(self.timer);
					$('.nTalk-btnHover').css('left', '709px');
					$('.nTalk-btnHover').css('display', 'block');

					$('.nTalk-btnHover-arrow').css({'background-position': '-303px -269px', 'left': '14px'});
				}

				$('.ntalk-window-body').css('width', currentWidth + speed + 'px');
				$('.ntalk-window-right').css('width', parseInt($('.ntalk-window-right').css('wdith')) + Math.abs(speed)  + 'px');

				$(".body-chat-containter").css('width',currentWidth + speed + 'px');
				$(".chat-view-window-history ul").css("width",currentWidth + speed -8 + 'px');
				$(".view-scrollBox").css("left",currentWidth + speed - 8 + 'px');
				//$('.chat-view-window-editor textarea').css('width',currentWidth + speed  + 'px')
				// $('.nTalk-btnHover').css('left',currentWidth + speed + 'px');
			}, 15)
