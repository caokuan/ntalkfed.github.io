;(function($, undefined){
	...
	$.chatView.prototype = {
         ...
         //2016.03.10 齐家网的定制
          _qijia_special:function(){
           ...
           // var qijia_ntalk_button_min=$('.ntalk-button-min');
			...
			var skq_ntalk_window_containter=$('.ntalk-window-containter');
			var skq_ntalk_window_right = $('.ntalk-window-right');
			var skq_window_right_tags = $('.window-right-tags');
			skq_window_right_tags.css({
				'border-radius': '6px',
				'-moz-border-radius': '6px',
				'-webkit-border-radius': '6px'
			})
			skq_ntalk_window_containter.css({
				'border-radius': '8px',
				        ...
				        if (type == 2) {
				            ...
				            $.require(data.url + '#image', function(image) {
				                if (image.error) {
				                    ...
				                    liElement.find('table').css('width', ( (width < 220 ? width : 220)) + 'px');
		...
		_showFileUpload: function(liElement, bodyElement, data, type) {
			...
			//IE7下布局调整
			liElement.find('table').css('width', '283px');
		...
		_getMessageHtml: function(type, data){
			...
			return type === 'right' ?
				[//发送的消息
					'<table style="',$.STYLE_NBODY,'float:right;_float:none;margin-right:5px;border-collapse:separate;" class="view-history-right" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'text-align:right;">',
							'<tr style="',$.STYLE_NBODY,'">',
							//2016.03.11更改颜色更改border 顏色值 添加color 值if(data.type == 7) data.type = 1;
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#014a8f;border:1px solid #014a8f;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">',
					...
					'<table style="',$.STYLE_NBODY,'float:left;float:none;table-layout:auto;border-collapse:separate;" class="view-history-left" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'">',
							'<tr style="',$.STYLE_NBODY,'">',
								...
								// 	'<div class="view-history-div" style="',$.STYLE_NBODY,'float:left;margin-left:8px; -margin-left:7px; width:40px; height:40px; display:inline-block; position:relative; top:0; background:url(',$.imageicon,') no-repeat -325px -47px; border-radius:50%">',
							   ...
							   '<td style="',$.STYLE_NBODY,'width:10px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
								...
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#ffffff;border:1px solid #ffffff;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px">',
									...
									(data.type == 1 ? 'color:#' + data.color + ';color:#5a5a5a;font:'+(data.italic=="true" ? 'italic' : 'normal')+' '+(data.bold=="true" ? 'bold' : 'normal')+' ' + data.fontsize + 'px/160% Arial,SimSun;color:#5a5a5a;text-decoration:' + (data.underline=="true" ? 'underline' : 'none') + ';' : ''),
									'">',
				...
				[
					...
					'<div class="chat-view-window-toolbar" style="',$.STYLE_BODY,'height:28px;width:100%;border-top:1px solid #ffffff;background:#ffffff;">',
						...
						'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
							'<div class="chat-view-window-status-info" style="',$.STYLE_BODY,'background:#002c58;overflow:hidden;color:#fff;margin-left:10px;width:380px;line-height:30px;height:30px;position:absolute;top:-30px;z-index:99;text-align:center;display:none;"></div>',
						'</div>',
					...
					'<div class="chat-view-window-editor" style="',$.STYLE_BODY,'height:60px;width:100%;overflow:hidden;">',
						//2016.03.22更改固定文字 更改颜色
						'<textarea placeholder="\u6211\u6709\u95ee\u9898\uff0c\u6211\u8981\u95ee\u002e\u002e\u002e" style="',$.STYLE_BODY,CON_STYLE_SHADOW,'margin:1px;padding:10px 10px 0 10px;width:391px;width:411px\\9;height:73px;height:93px\\9;outline:0px solid #08f;border:0px solid #08f;color:#999999;font-family:Arial;resize:none;overflow-x:hidden;overflow-y:auto;"></textarea>',
					'</div>',
					...
					/*	'<div class="chat-view-options" style="',$.STYLE_BODY,'margin:6px 10px 6px 0;float:right;border:1px solid #ccc;width:14px;height:26px;line-height:25px;text-align:center;cursor:pointer;">',
							...
							'</div>',
						'</div>',*/
						'<div class="chat-view-submit" style="',$.STYLE_BODY,'margin:7px 0;margin-left: 6px;float:right;width:auto;height:26px;line-height:26px;text-align:center;padding:0 24px;border:1px #CCC solid;border-right:none;cursor:pointer;">'
						//2016.03.22更改固定字体
						,'\u53d1\u9001\u0028\u0053\u0029',
						'</div>',
						//2016.03.10 修改背景颜色' ,$.lang.button_end_session,'
						'<span class="chat-view-end-session" style="',$.STYLE_BODY,'border:1px solid #ebedec;color: #3b3b3b;text-decoration:none;margin:7px 10px 7px 0;padding:0 10px;float:right;height:26px;line-height:26px;cursor:pointer;background-color:#f3f3f3;">\u7ed3\u675f\u5bf9\u8bdd</span>',
						//2016.03.22修改颜色
	...
	$.chatManageView.prototype = {
		...
		updateChatTag: function(settingid, data, updateStatus){
			...
			this.leftContent.find('li.'+settingid+' .tag-head-icon').css('background-color', data.status!==1 ? '#666' : '#002C58');
		...
		_create: function(){
			...
			this._objView.buttonClose.hover(function(){
				//$(this).css('background-position', '-60px -20px');
				//2016.03.15
				$(this).css('background-color','#014a8f');
			}, function(){
				//$(this).css('background-position', '-60px 0');
				$(this).css('background-color','');
				...
				background: 'url(' + $.imageicon + ') no-repeat -57px 12px'
			});
			...
			this._objView.buttonMax.hover(function(event){
				...
				width:'30px',
				height: '40px',
				background: 'url(' + $.imageicon + ') no-repeat -34px 12px'
			...
			this._objView.buttonMin.hover(function(){
				...
				height: '40px',
				//2016.03.30 
				background: 'url(' + $.imageicon + ') no-repeat 10px 12px'
			...
			this.headBody = $({className: 'chat-header-body', style: $.STYLE_BODY + 'background:#ebe9e9;z-index:0;color:#525252;'}).appendTo(this.header, true).css({
				...
				'border-bottom': '0',
				'top': '0px',//IN
				'border-right': '1px solid #002c58',//IN
			...
			this.headPic=$({tag: 'span', className:'chat-header-pic', style: $.STYLE_BODY + 'display:inline-block;margin:14px 0px 0px 15px;padding-right: 10px;border-right:1px solid #fff;height:13px;width:142px;float:left;background:url('+ $.imageicon + ') no-repeat -287px -102px' }).appendTo(this.headBody).html('');
			//2016.03.11更改字体颜色和margin 值
			this.headName = $({tag: 'span', className:'chat-header-name', style: $.STYLE_BODY + 'color:#ebedec;margin:13px 0px 0px 0px;padding-left: 5px;display:inline-block;heigth:13px;color:#ffffff;font-size:12px;float:left;max-width:220px;visibility:hidden;overflow:hidden;cursor:auto;'}).appendTo(this.headBody);
			...
			this.rightBody = $({className: 'window-right-body', style: $.STYLE_BODY + 'width:'+(this.options.rightElementWidth-1 - 14)+'px;background:#fff;'}).css({
				'padding': '0px 7px 0',
				'border-right':  '1px solid #f3f3f3',
				'border-bottom': '1px solid #f3f3f3',
				'height':+ (this.options.height - 1 - 210) + 'px',
				'border-radius': '0px 0px 0px 0px',
				'-moz-border-radius': '0px 0px 0px 0px',
				'-webkit-border-radius': '0px 0px 0px 0px'
			}).appendTo(this.rightElement);
			// 2016.03.21添加iframe框架
			var iframeUrl = 'http://service.skechers.cn/im.html' +'?'+ $.toURI({
				uid: $.user.id || ''
			})
			var pptElement_tem_height=this.options.height;
			this.pptElement = $({tag: 'iframe', className: 'window-right-pro-conent', style: $.STYLE_BODY + 'margin:10px 0 0 0;width:' + (this.options.rightElementWidth-15) + 'px;background:#fff;height:'+ pptElement_tem_height+'px;overflow-x:hidden;overflow-y:hidden;'}).attr({
				//debug temp
				//别人提供的地址
		...
		_callResize: function(args){
			...
			this.rightElement.find('.window-right-pro-conent').css({'height': rigthConentHeight + 'px'});
		...
		_callMaxResize: function(){
			...
			if( setMax ){
				this._objView.buttonMax.css('background-position', '-12px 12px').attr('title', $.lang.chat_button_resize_min);
			}else{
				this._objView.buttonMax.css('background-position', '-34px 12px').attr('title', $.lang.chat_button_resize_max);
			}
		...
		_addRightLabel: function(title, data, length, selected){
			var self = this,
				...
				rightContentWidth  = this.options.rightElementWidth - 16 - length + 1,
				//2016.03.21更改高度
				rightContentHeight = this.options.height -314;
			
			if( !this.rightTags ){
				//2016.03.11更改颜色 border 改为1
				this.rightTags = $({className:'window-right-tags', style: $.STYLE_NBODY + 'background:#ffffff;z-index:-1;overflow:hidden;height:34px;border-top:1px solid #FFF;border-left:1px solid #fff;border-right:1px solid #fff;border-radius: 6px;-moz-border-radius: 6px;-webkit-border-radius: 6px;'}).appendTo(this.rightBody);
			...
			if( length == 1 ){
				...
				if( this.rightTags.find('div').length == 1 ){
					...
					style += 'width:' + (length == 2 ? (rightContentWidth-4)/2 : 87) + 'px;border-right:1px solid #ebedec;';
				}else if( this.rightTags.find('div').length < length ){
					style += 'width:' + (length == 2 ? (rightContentWidth-4)/2 : 87) + 'px;border-right:1px solid #ebedec;';//border-left:1px solid #FCE4E7;
				}else{
					//2016.03.29 修改大小
					style += 'width:' + (length == 2 ? (rightContentWidth-4)/2 : 87) + 'px;';//border-left:1px solid #FCE4E7;
				}
			}
			if(title.indexOf("\u5173\u4E8E\u4F01\u4E1A") > -1) {
				title = '\u54C1\u724C\u54A8\u8BE2';
			}
			if( title.indexOf("\u552e\u540e\u670d\u52a1") > -1 ){
				title = '\u5168\u56FD\u7F51\u70B9';
			}
			...
			//}
			else if(title.indexOf("\u54C1\u724C\u54A8\u8BE2") > -1){
				$({className: 'window-right-consultation', style: $.STYLE_BODY + 'margin:5px;word-wrap:break-word;overflow:hidden;'}).appendTo(tagBody);
				$({className:'nTalk-consultation-logo',tag:'div',style: $.STYLE_BODY + 'width:250px;height:70px;margin-top:11px;position: relative'}).appendTo(".window-right-consultation");
				$({className:'nTalk-consultation-logo-left',tag:'div',style: $.STYLE_BODY+'width:68px;height:68px;border-radius:50%;display:inline-block;'}).appendTo('.nTalk-consultation-logo');
				$({className:'nTalk-consultation-logo-right',tag:'div',style: $.STYLE_BODY+'width: 160px;height:50px;display:inline-block;marign-left: 15px;position: absolute;'}).appendTo('.nTalk-consultation-logo');
				$({className:'nTalk-consultation-logo-right-top',tag:'span',style: $.STYLE_BODY+'width: 100%;height:14px;display:block;color:#5a5a5a;'}).appendTo('.nTalk-consultation-logo-right');
				$({className:'nTalk-consultation-logo-right-bottom',tag:'span',style: $.STYLE_BODY+'width: 48px;height:16px;display:block;margin-top:10px;'}).appendTo('.nTalk-consultation-logo-right');
				$('.nTalk-consultation-logo-right').css({
					'top': '20px',
					'left': '85px'
				})
				$('.nTalk-consultation-logo-left').css({
					'background': 'url(' + $.themesURI + 'skq_logo.png' + ')',
					'background-repeat' : 'no-repeat',
					'background-position': 'center center'
				})
				$('.nTalk-consultation-logo-right-top').insert('SKECHERS\u65AF\u51EF\u5947');
				$('.nTalk-consultation-logo-right-bottom').css({
					'background': 'url(' + $.themesURI + 'v.png' + ')',
					'background-repeat' : 'no-repeat',
					'background-position': 'center center'
				})
				
			}
			else if( title.indexOf("\u5168\u56FD\u7F51\u70B9") > -1){
				title = "\u5168\u56FD\u7F51\u70B9";
				getgetprovince = function(){
					callbackname = 'callback_' + $.randomChar();
					queryString = $.toURI({
						"country":1,
						callback: callbackname
					});
					$({tag:'option',value:"",selected:'true'}).appendTo(".nTalk-getprovince").insert('省份/直辖市');
					window[callbackname] = function(result){
						var arr = [];
						if( result.msg ==  "success" ){
							for( var i=0 ; i<result.data.length ; i++ ){
								$({tag:'option',value:result.data[i].id}).appendTo(".nTalk-getprovince").insert(result.data[i].name);
								//arr.push( '<option value="'+result.data[i].id+'">'+result.data[i].name+'</option>')
							}
						}

						//var str = arr.join("");
						//console.log(str);
						//  $(".nTalk-getprovince").html(str);
					}
					$.require("https://ntkf.wbiaotest.cn/region/province" + '?' + queryString +"#rnd");
					getcity("");
				}
				getcity = function( value ){
					if( value == "" ){
						$({tag:'option',value:""}).appendTo(".nTalk-getcity").insert('城市');
					}else{
						callbacknamecity = 'callback_' + $.randomChar();
						queryStringcity = $.toURI({
							"province":value,
							callback: callbacknamecity
						});
						$({tag:'option',value:"",selected:'true'}).appendTo(".nTalk-getcity").insert('城市');
						window[callbacknamecity] = function(result){
							if( result.msg ==  "success" ){
								for( var i=0 ; i<result.data.length ; i++ ){
									$({tag:'option',value:result.data[i].id}).appendTo(".nTalk-getcity").insert(result.data[i].name);
								}
							}
						}
						$.require("https://ntkf.wbiaotest.cn/region/city/" + '?' + queryStringcity+"#rnd");
					}
					getstore("");
				}
				getstore = function(value){
					if( value == "" ){
						$({tag:'option',value:"",selected:'true'}).appendTo(".nTalk-getstore").insert('合作维修点');
					}else{
						callbacknamestore= 'callback_' + $.randomChar();
						queryStringstore = $.toURI({
							"p":$('.nTalk-getprovince').value,
							"c":value,
							callback: callbacknamestore
						});
						$({tag:'option',value:"",selected:'true'}).appendTo(".nTalk-getstore").insert('合作维修点');
						window[callbacknamestore] = function(result){
							var arr = [];
							if( result.msg ==  "success" ){
								for( var i=0 ; i<result.data.length ; i++ ){
								$({tag:'option',value:result.data[i].name+'+ntalk'+result.data[i].address+'+ntalk'+result.data[i].phone}).appendTo(".nTalk-getstore").insert(result.data[i].name);
								}
							}
						//var str = arr.join("");
						//$(".nTalk-getstore").html(str);
						}
						$.require("https://ntkf.wbiaotest.cn/service/store/" + '?' + queryStringstore+"#rnd");
					}
				}

				$({className: 'window-right-text', style: $.STYLE_BODY + 'margin:5px;word-wrap:break-word;overflow:hidden;'}).appendTo(tagBody);
				$({className:'nTalk-getprovince',tag:"select",value:'省',style:"margin:0;padding:0;width:240px;height:33px;border:1px solid #e4e4e4;color:#666;margin-bottom:10px;outline:none;display:block;float:left;border-radius:3px;"}).appendTo(".window-right-text");
				$({className:'nTalk-getcity',tag:"select",style:"margin:0;padding:0;width:240px;height:33px;border:1px solid #e4e4e4;color:#666;margin-bottom:10px;outline:none;display:block;float:left;border-radius:3px;"}).appendTo(".window-right-text");
				$({className:'nTalk-getstore',tag:"select",style:"margin:0;padding:0;width:240px;height:33px;border:1px solid #e4e4e4;color:#666;margin-bottom:10px;outline:none;display:block;float:left;border-radius:3px;"}).appendTo(".window-right-text");
				$({className:'nTalk-storeDetial',tag:'div',style:'margin:0;padding:0;width:240px;height:auto;overflow:hidden;border-top:1px dashed #e4e4e4;float:left;float:left;padding-top:10px;'}).appendTo(".window-right-text");
				getgetprovince();
				$('.nTalk-getdetial').bind('click',function(){

				})
				$('.nTalk-getprovince').bind('change',function(){
					$('.nTalk-storeDetial').find("*").remove();
					$('.nTalk-getcity').find("*").remove();
					$('.nTalk-getstore').find("*").remove();
					getcity(this.value);

				});
				$('.nTalk-getcity').bind('change',function(){
					$('.nTalk-storeDetial').find("*").remove();
					$('.nTalk-getstore').find("*").remove();
					getstore(this.value);
				});
				$('.nTalk-getstore').bind('change',function(){
					$('.nTalk-storeDetial').find("*").remove();
					var arrDetial = $(".nTalk-getstore").get(0).value.split("+ntalk");
					var detailstrarr = [];
					if( arrDetial.length > 0 ){
						for( var i=0 ; i<arrDetial.length ; i++){
							detailstrarr.push('<span>'+arrDetial[i]+'</span><br/>');
						}
						$('.nTalk-storeDetial').html(detailstrarr.join(""));
					}
				});
		...
		_clearTag: function(){
			this.rightBody.find(".window-right-pro-conent *,.window-right-pro-conent *").remove();
			this.rightTags = null;
