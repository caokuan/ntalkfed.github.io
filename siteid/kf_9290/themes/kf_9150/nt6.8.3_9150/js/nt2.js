;(function($, undefined){
    ...
    $.extend({
        ...
        listenerMessage: function(bindFunc){
            ...
            window.addEventListener("message", funcMessage);
	...
	$.Window.prototype = {
		...
		_create: function(){
			...
			this.buttonClose = $({className: 'ntalk-button-close', style: $.STYLE_BODY + 'width:20px;height:20px;cursor:pointer;position:static;float:right;position:relative;margin:2px 3px 0 0;line-height:20px;vertical-align:middle;background:none;'}).appendTo( this.header );
			this.buttonMax = $({className: 'ntalk-button-maxresize', style: $.STYLE_BODY + 'width:20px;height:20px;cursor:pointer;position:static;float:right;position:relative;margin:2px 3px 0 0;line-height:20px;vertical-align:middle;background:none;'}).appendTo( this.header );
			this.buttonMin = $({className: 'ntalk-button-min', style: $.STYLE_BODY + 'width:20px;height:20px;cursor:pointer;position:static;float:right;position:relative;margin:2px 3px 0 0;line-height:20px;vertical-align:middle;background:none;'}).appendTo( this.header );

			//left
			this.body = $({className:'ntalk-window-body', style:$.STYLE_BODY + 'float:left;width:100%;'}).appendTo(this.containter);

			this.chatBody  = $({className: 'ntalk-chat-body', style: $.STYLE_BODY + 'width:100%;position:relative;left:0;top:0;'}).appendTo( this.body );

			//right
			if(this.rightNode){
				this.rightElement = $({className:'ntalk-window-right', style:$.STYLE_BODY + 'float:left;display:none;width:100%;'}).appendTo(this.containter);
			}

			if( this.resize ){
				this.buttonResize = $({className: 'window-resize', style: $.STYLE_BODY + 'width:10px;height:10px;cursor:nw-resize;position:absolute;right:1px;bottom:1px;font-size:0;background:none;z-index:99;'}).appendTo( this.containter );
			}

			$({style: $.STYLE_BODY + 'clear:both;'}).appendTo(this.containter);
	...
	$.pageManage.prototype = {
		...
		initialize: function(options, siteid){
			var self = this, arrPageList, cookiePageList, LSReadCount = 3;
			...
			if($.browser.chrome){this.options.timeout = 5;}	//chrome浏览器多页卡会有定时器紊乱现象
		...
		_update: function(intervalTime){
			...
			var self = this, type = "update", Index = this._getIndex();
	...
	$.store = (function(){
		...
		if( isStorageSupported() ){
			...
			store.set = withIEStorage(function(storage, key, val) {
				...
				try { storage.save('nTK-LS'); }catch(e){}
				return val;
			});
	...
	$.comet.prototype = {
			...
			if( !this.connectionPooling ){
				...
				for(var i = 0; i <= this.options.muDomain; i++){
					this.connectionPooling.enQueue({
						uri:    i === 0 ?
							this.uri.toString().replace(/(https?:\/\/)(.*?)(\-\d+)?\./ig, "$1$2.") :
							this.uri.toString().replace(/(https?:\/\/)(.*?)(\-\d+)?\./ig, "$1$2-" + (+i - 1) + "." ),
						lock:       false,
						sTimesample:0,
						rTimesample:0
					});
		...
		connect: function(json, callName){
			...
			this.connectOptions = $.extend(json, {ts: $.getTime()});
		...
		kalive: function(json, callName){
			...
			this.kaliveOptions = $.extend(this.kaliveOptions, json, {ts: $.getTime()});
		...
		disconnectServer: function(json, cache){
			...
			this.flashGoServer = objConnect.url + $.toURI( cache===false ? json : $.extend(json, {ts: $.getTime()}) );
		...
		_createConnect: function(uri/*:URl*/, id/*:String*/, index/*:number*/, connect/*:object*/){
			var self = this, script, eventName,
    ...
    $.extend({
		...
		utils: {
			options: {},
			handleLinks: function(html, options) {
			...
			linkPatterns: [
				...
				[
					/\{\$(\w+)}/gi,
					function(a, n){return $.utils.options[n] || '';}]
			]
		},
		toHSL: function(c){
		...
		hex2RGB: function(sHex){
			var c = sHex.toString().replace('#', ''), a = c.split(''), oRGB = {};
			if( c.length==3 ){
				oRGB = {r: parseInt(a[0]+a[0], 16),
						g: parseInt(a[1]+a[1], 16),
						b: parseInt(a[2]+a[2], 16)};

			}else if( c.length==6 ){
				oRGB = {r: parseInt(a[0]+a[1], 16),
				...
				return $.each(this, function(i, element){
					if( $.browser.oldmsie ){
						...
						if( /right|bottom/.test(direction) ){
							c = scolor; scolor = ecolor; ecolor = c;
						}
					}
					if( $.browser.webkit ){
						switch(direction){
							case 'top':dp='0% 100%,0% 0%';break;
							case 'right':dp='0% 0%,100% 0%';break;
							case 'bottom':dp='0% 0%,0% 100%';break;
							case 'left':dp='100% 0%,0% 0%';break;
						}
	...
	$.extend({
		...
		FORM: { //form
			...
			createInput: function(cfgRequiredField, appendTdParams, noNullChar){
				var trHtml = [], formField, option, checked, formTitle,
					mobile = $.browser.mobile,
					appendTd = $.extend({id: '', rowspan:0, style:''}, appendTdParams),
						...
						case 'radio':
							...
							for(var j=0, radioId; j<formField.options.length; j++){
								...
								trHtml.push('<li class="form-item" style="' + $.STYLE_BODY + 'list-style:none;padding:0 2px 0 0;color:#000;float:left;">' +
									'<input type="radio" name="' + formField.name + '"id="' + radioId + '" value="' + option.value + '" _custom_text="' + option.text + '" style="' + $.STYLE_BODY + 'color:#333;"' + checked + ' />' +
			...
			bindFormEvent: function(cfgRequiredField, parent){
				$(parent).find('input[type=text], textarea').bind('focus', function(){
					var element = $(this).css({'color': '#333', 'border-color': $.browser.mobile ? '#0079fe' : '#666'}),
			...
			verificationForm: function(fields, fnCallback, parentElement, failCallback){
				var element, selectElem, isNull, returnValue = [],showNews,value,
				    failFlag = false,
			...
			showError: function(showNews, message, info, infodiv, element, type){
				...
				if( showNews && message ){
					if(!$.browser.mobile){
						...
						this.messageErrorToast = new $.Toast('<div id="#message_error" style="position: relative;width: ' + (width-50) + 'px; height:30px; line-height: 30px;z-index:100; color: #FFF; top: 30px; left: 25px; text-align:center;font-weight:bold">'+message+'</div>', {width:width, height:90});
	...
	*/
	...
	$.Transfer.prototype = {
		...
		initialize: function(options, button){
			...
			this.proxy = $({tag:'IFRAME', name:"proxy-" + tmpName, src: this.options.server.substring(0, this.options.server.lastIndexOf('/')) + '/proxy.html?t=' + $.getTime(), style:$.STYLE_NBODY + 'width:0px;height:0px;display:none;'}).appendTo($(this.button)).get(0).contentWindow;
			...
			this.form	= $({tag:'FORM', action:'', method:'POST',target: this.options.target,enctype:'multipart/form-data',style:style}).appendTo( this.button, true );
			this.iframe	= $({tag:'IFRAME', name:this.options.target, src:'about:blank', style:style + 'width:0;height:0;display:none;'}).appendTo( this.button, true );
			//允许所有类型的文件
			this.element= $({tag:'INPUT', type:'file', name:this.options.name, accept:this.options.accept || '*', style:style, title: this.button.attr('title') || ''}).appendTo( this.form, true ).css('opacity', 0);
		...
		commonTransf : function(event, files) {
			...
			if( typeof files === 'string' ){
				...
				for(var reg, i = 0; i < files.length; i++){
					...
					if( (this.options.maxSize && file.size > this.options.maxSize) || (proxyMaxSize && file.size > proxyMaxSize) ){
						this.options.onError({type:9, name:file.name, size:file.size, etype:'SIZE', maxSize: this.options.maxSize || proxyMaxSize});
						return;
					}
					if( (this.options.accept == '*' || !this.options.accept) && !proxyExt ){
						...
						if( !reg.test(file.type) ){
							...
							this.options.onError({type:9, name:file.name, size:file.size, etype:'TYPE'});
							return;
						}
						...
						this.options.onError({type:9, name:file.name, size:file.size, ext:proxyExt, etype:'TYPE'});
						return;
					}
			...
			//手机上需要发送正在上传状态
			if( $.browser.mobile ) {
				this.options.callback({status: 'startUpload', oldfile: files[0].name});
			}
		...
		mobileCompTransf : function(event, files) {
			var self = this;
			this.options.callback({status: 'startCompress', oldfile: files[0].name});
			//获取图片方向后，回调生成压缩图片方法
			var Oriention = new $.ImageOrientation(files[0], function(event, orientation){
				//获得压缩后的图片的base64 url后，发送到服务端
				var compImg = new $.CompressImg(files[0], {'orientation': orientation }, function(event, dataurl){
					//使用domain+iframe方式，跨域往服务器上传送
					//传送的参数
					self.formData = new FormData();
					self.formData.append('base64', dataurl);
					self.formData.append('fname',  $.getTime()+'.jpg');

					self.options.action = 'uploadimage';
					self.options.siteid = self.options.params.siteid;
					$.each(self.options, function(name, value){
						self.formData.append(name, value);
					});
					self.uploadCallBack =  function(data){
						if(data.status == 'success'){
							data = data.event.target.responseText;
							self.options.callback($.JSON.parseJSON(data));
						}else if(data.status == 'error'){
							self.options.callback('error');
						}
				    };
					self.options.callback({status: 'startUpload', oldfile: files[0].name, compress: true});
					//调用proxy.html里的uploadFile方法上传文件
					try{
						self.proxy.uploadFile(self.options.server, self.formData, self.uploadCallBack);
					}catch(e){
						$.Log('base64 upload' + e.message, 3);
					}
				});
			});
	...
	$.DialogChat.prototype = {
		...
		_create: function(){
			...
			this.iframe = $({tag:'IFRAME', className: 'dialog-iframe-' + this.id, style:$.STYLE_NBODY + 'position:' + position + ';display:none;left:0;top:0;margin:0;padding:0;border:0;width:100%;z-index:8888;'}).appendTo(body);
			this.background = $({tag:'div', className: 'dialog-background-' + this.id, style:$.STYLE_NBODY + 'position:' + position + ';left:0;top:0;margin:0;padding:0;border:0;width:100%;background-color:#000;z-index:8888;'}).appendTo(body);
			var zIndex = $.browser.mobile ? 9000 : 	2147483647;
			this.container = $({tag:'div', className: 'dialog-container-' + this.id, style:$.STYLE_BODY + 'position:' + position + ';left:0;top:0;margin:0;min-height:'+(this.options.minHeight ? this.options.minHeight : 200)+'px;border-radius:0px;z-index:'+zIndex+';background:#fff;overflow-x:hidden;overflow-y:hidden;'}).appendTo(body);
		...
		_style: function(style){
			...
			this.background.css({opacity: 0.6});
