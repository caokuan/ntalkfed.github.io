(function($, undefined) {
 ...
 //2016.04.18
    ...
    $.title_image=$.sourceURI + 'images/wapTheme3/title.png';
    var newWapTheme = {
        title: "#fbc7a8",
        return_style: "&#xe618;",
        return_color: "#513727",
    ...
    chatView.prototype = {
        ...
        _create: function() {
            ...
            this.chatElement = $({
                    ...
                    style: $.STYLE_BODY + 'width:100%;/*min-height:' + bodyHeight + 'px;*/display:none;overflow-y:scroll;position:absolute;top:'+header_title_height+'px;bottom:50px;-webkit-overflow-scrolling:touch;overflow-x:hidden;'+background_whole_exp+''
                        ...
                        $.require(data.sourceurl + '#image', function(image) {
                            if (image.error) {
                                ...
                                bodyElement.css({
                                    ...
                                    'border':''
                                }).html('<img src="' + data.sourceurl + '" sourceurl="' + data.sourceurl + '" width="' + attr.width + '" height="' + attr.height + '" style="' + $.STYLE_BODY + 'width:' + attr.width + 'px;height:' + attr.height + 'px;vertical-align:middle;border-radius:5px;" />');
                            }
                        });
                                ...
                                if (image.error) {
                                   ...
                                   //2015.05.15 发送失败时刷新页面时的显示结果
                                    ...
                                    bodyElement.css({
                                        ...
                                        //2016.04.11更改padding值
                                        'padding': '0px',
                                        'background': backgroundColor,
                                        'cursor': 'pointer',
                                        'height': attr.height + 'px'
                                    }).html('<img src="' + data.url + '" sourceurl="' + data.sourceurl + '" width="' + attr.width + '" height="' + attr.height + '" style="' + $.STYLE_NBODY + 'vertical-align:middle;border:solid 1px #ffffff;border-color:'+img_border+';border-radius:5px;" />').find('img').tap(function(event) {
        ...
        viewLinkContainer: function(data, selector) {
            ...
            linkImage.css('margin-top', (root.height()-linkImage.height())/2);
        ...
        _outTimeResend: function(msgid) {
            ...
            bodyEl.css('background-position', '-350px -6px');
        ...
        _showFailure: function(msgid, eType) {
            ...
            return bodyElement.css('background', 'url(' + $.imageicon + ') -350px -6px transparent');
        },
...
/*            dialogElement.find('table textarea').bind('keyup', function(event) {
        ...
        _createShowWebIframe: function(url) {
            ...
            //变更标题框
            //self.header.css({'background-color':'#32A8F5', 'height':'44px'});
        ...
        _getMessageHtml: function(type, data) {
            ...
            return type === 'right' ? [ //发送的消息
                   //2016.04.08 更改margin 值使其距离上下为4px;
                    ...
                    (
                        //发送文件时
                        4 == data.type ? ['<div class="view-history-body" type="', data.type, '" style="', $.STYLE_NBODY, 'word-break:break-all;word-wrap:break-word;padding:8px;border-top-left-radius:5px;border-top-right-radius:5px;border-bottom-left-radius:5px;border-radius:5px;background:'+bubble+';border:1px solid #a1cce8;border-color:'+bubble+'">',
                        ...
                        6 == data.type ? ['<div class="view-history-body" type="', data.type, '" style="', $.STYLE_NBODY, '">',
                            '<div class="view-history-audio" style="', $.STYLE_BODY, 'float:left;min-width:1px;height:30px;line-height:30px;overflow:hidden;padding-left:6px;padding-right:40px;background:'+bubble+';border:1px solid #a1cce8;border-color:'+bubble+';border-top-left-radius:5px;border-top-right-radius:5px;border-bottom-left-radius:5px;border-radius:5px;">',
                            '<div class="view-audio-voice" style="position:relative; right:-40px; height:40px;line-height:40px">',
                                '<svg width="32px" height="32px">',
                                '<path d="M17 6 C 11 12, 11 20, 17 26" stroke="#55a2d5" fill="transparent" stroke-width="2"></path>',
                                '<path d="M20 10 C 16 14, 16 18, 20 22" stroke="#55a2d5" fill="transparent" stroke-width="2"></path>',
                                '<path d="M24 12 C 21 14, 21 18 24 20" stroke="#55a2d5" fill="transparent" stroke-width="2"></path>',
                                '</svg>',
                            '</div>',
                            '<div class="view-audio-voice-duration" style="position:relative; right:0px; top:-44px;height:40px;line-height:40px">',
                            '</div>',
                            '</div>',
                            '</div>'
                        ].join('') :
                        //消息或图片
                        //2016.04.06
                        ['<div class="view-history-body" type="', data.type, '" style="', $.STYLE_BODY, 'line-height:160%;', (data.type == 2 ? 'color:'+bubble_text+';text-align:center;display:table-cell;*display:inline-block;vertical-align:middle;width:75px;height:75px;*font-size:0px;*line-heihgt:0px;*font-family:Arial;padding:2px;background:none;border-top-left-radius:5px;border-top-right-radius:5px;border-bottom-left-radius:5px;' : 'display:block;'), 'word-break:break-all;word-wrap:break-word;padding:6px;color:'+bubble_text+';background:'+bubble+';border:1px solid #a1cce8;border-color:'+bubble+';border-top-left-radius:5px;border-top-right-radius:5px;border-bottom-left-radius:5px;margin-left:50px;">', (/^(1|9)$/i.test(data.type) ? data.msg : /^2$/i.test(data.type) && data.oldfile ? data.oldfile : ''),
                            '</div>',
                         '<div class="view-history-pic-status" style="position: relative; top: -24px; font-size:12px;margin:0px; left:-2px; padding:0px; text-align:center; display:none"></div>'
                        ].join('')
                    ),
                    //==相关状态==
                    ...
                    '</td>',
                    //==尖角==
                    //(data.type == 2 ? ['<td class="view-chat-angle" valign="top">',
                    //'<span style="display:none;position:relative;display:block; height:0px; width:0px; font-size:0; margin-top:13px; line-height:0;border-left:0px;border-top:3px solid transparent;border-left:6px solid #CEF2FF;border-left-color:'+bubble+';border-bottom:3px solid transparent;margin-right:5px;z-index:100;left:-1px"></span>',
                   // '<span style="display:none;position:relative;top:-20px;display:block; height:0px; width:0px; font-size:0; margin-top:13px; line-height:0;border-left:0px;border-top:4px solid transparent;border-left:8px solid #a1cce8;border-left-color:'+bubble_border+';border-bottom:4px solid transparent;margin-right:5px;z-index:50"></span>',
                   // '</td>'].join("") :
                    //['<td valign="top">',
                    //'<span style="position:relative;display:block; height:0px; width:0px; font-size:0; margin-top:13px; line-height:0;border-left:0px;border-top:3px solid transparent;border-left:6px solid #CEF2FF;border-left-color:'+bubble+';border-bottom:3px solid transparent;margin-right:5px;z-index:100;left:-1px"></span>',
                    //'<span style="position:relative;top:-20px;display:block; height:0px; width:0px; font-size:0; margin-top:13px; line-height:0;border-left:0px;border-top:4px solid transparent;border-left:8px solid #a1cce8;border-left-color:'+bubble_border+';border-bottom:4px solid transparent;margin-right:5px;z-index:50"></span>',
                    //'</td>'].join("")),
                    ...
                    //2016.04.08更改
                    '<tr>',
                    (data.type == 2 ? ['<td class="view-chat-angle" valign="top" style="display:none;float:right;position:relative;top:-1px;right:0;">',
                    '<svg height="5px" width="7px" id="svg" >',
                    '<path d="M0,0 L7,0 L7,5 C5,4 2,1 0,0" style="stroke-width:1;fill:'+bubble+';stroke:'+bubble+';"/>' ,
                    '</svg>',

                   '</td>'].join("") :
                    ['<td valign="top" style="float:right;position:relative;top:-1px;right:0;">',
                   '<svg height="5px" width="7px" id="svg" >',
                    '<path d="M0,0 L7,0 L7,5 C5,4 2,1 0,0" style="stroke-width:1;fill:'+bubble+';stroke:'+bubble+';"/>' ,
                    '</svg>','</td>'].join("")),
                ...
                /left|bottom/gi.test(type) ? [ //接收的消息
                      //2016.04.13
                    '<table style="', $.STYLE_NBODY, 'margin:4px 0 0 0;width:100%;" class="view-history-left" cellpadding="0" cellspacing="0" border="0" class="table">',
                    '<tbody style="', $.STYLE_NBODY, 'text-align:center;">',
                    ...
                    //2016.04.08 删除其头像和客服名称
                   // '<td rowspan="2" style="', $.STYLE_NBODY, 'vertical-align:top;text-align:center;width:40px;_width:40px;height:auto;min-height:40px;">',
                   // '<div class="view-history-icon-div" style="', $.STYLE_NBODY, 'text-align:center;width:35px;height:35px;border-radius:5px;border:1px solid #d4d4d4;background:url(', $.imageicon, ') no-repeat -307px -55px;overflow:hidden;">', (data.logo ? ['<img data-single="1" onerror="nTalk.loadImageAbnormal(this, event)" class="view-history-receive-icon" userid="', data.userid, '" src="', data.logo, '" style="', $.STYLE_NBODY, 'width:35px; height:35px" />'].join('') : ''),
                   // '</div>',
                   // '</td>',
                    //客服名称
                  ...
                  //  '</tr>',
                    '<tr>',
                    //==尖角==
                    ['<td valign="top" class="view-chat-angle" style="width:auto;', data.type == 2 ? 'display:none;' : '','">',
                     '<svg height="5px" width="7px" id="svg" >',
                    '<path  d="M0,0 L0,5 L7,5 C5,4 2,1 0,0" style="stroke-width:1;fill:#ffffff;stroke:#ffffff;"/>' ,
                    '</svg>',
                    '</td>'].join(""),
                    '</tr>',
                    '<tr>',
                    ...
                    (
                        //收到文件时2016.04.11更改border 颜色
                        4 == data.type ? ['<div class="view-history-body" style="', $.STYLE_BODY, 'word-break:break-all;word-wrap:break-word;border-top-right-radius:5px;border-bottom-left-radius:5px;border-bottom-right-radius:5px;padding:8px 0;background:white;border:1px solid #ffffff;">',
                            //上面的整个div
                        ...
                        //消息或图片
                        ['<div class="view-history-body" type="', data.type, '" style="', $.STYLE_BODY, '', (data.type == 2 ? 'color:#6C6C6C;text-align:center;display:table-cell;*display:inline-block;vertical-align:middle;width:75px;height:75px;*font-size:0px;*line-heihgt:0px;*font-family:Arial;' : 'display:block;'), 'word-break:break-all;word-wrap:break-word;padding:6px;background:white;border-top-right-radius:5px;border-bottom-left-radius:5px;border-bottom-right-radius:5px;color:#6C6C6C;border:1px solid #ffffff;margin-right:52px">',
                            //==消息
        ...
        _getViewHtml: function(type) {
            ...
            $.face_image= $.sourceURI + 'images/wapTheme2/xiaolian.png';
            $.face_image1= $.sourceURI + 'images/wapTheme2/xiaolian1.png';
            $.button_action_image= $.sourceURI + 'images/wapTheme2/tianjia.png';
            $.button_action_image1= $.sourceURI + 'images/wapTheme2/tianjia1.png';
            $.evaluate_image= $.sourceURI + 'images/wapTheme2/pingjia.png';
            $.evaluate_image1= $.sourceURI + 'images/wapTheme2/pingjia1.png';
            $.pic_image= $.sourceURI + 'images/wapTheme2/image.png';
            $.pic_image1= $.sourceURI + 'images/wapTheme2/image1.png';
            $.audio_image=$.sourceURI + 'images/wapTheme2/yuyin.png';
            $.audio_image1=$.sourceURI + 'images/wapTheme2/yuyin1.png';
            $.jianpan_image=$.sourceURI + 'images/wapTheme2/jianpan.png';
            $.jianpan_image1=$.sourceURI + 'images/wapTheme2/jianpan1.png';
           //2016.05.06 添加发送按钮的背景颜色
            ...
            return type == 'load' ? [
                ...
                type === 'window' ? [
                    //分配客服时状态消息
                    '<div class="chat-view-window-status-info" style="', $.STYLE_BODY, 'overflow:hidden;background:#f9efda;width:100%;line-height:55px;height:55px;position:fixed;top:',($.browser.iphone ?'0' :'44px'),';display:none;z-index:300">',
                    ...
                    '<ul style="', $.STYLE_BODY, 'list-style:none;margin:20px 13px 0 13px;background:transparent">',
                    //'<li style="',$.STYLE_BODY,'list-style:none;margin:0;"></li>',
                    //2016.04.08

                    //2016.04.08 增加客服的头像
                   '<div class="view-history-icon-div" style="', $.STYLE_NBODY, 'text-align:center;width:35px;height:35px;border-radius:50%;border:1px solid #ffffff;background:url(', $.imageicon, ') no-repeat -307px -55px;overflow:hidden;">',
                    '</div>',
	 ...
	 * @param  {boolean} hidden    隐藏|显示控制区域
        ...
        setDescIcon: function(parentElement, userid) {
            ...
            if (!dest || this.mode.isVisitor(userid) || !dest.logo) {
                //2016.04
                //$('.view-history-icon-div .view-history-receive-icon').remove();
            } else {
                    ...
                    if (image.error !== true) {
                        ...
                        if (!icon.length) {
                            icon = $({
                                ...
                                style: $.STYLE_NBODY
                            }).appendTo($('.view-history-icon-div'));
                        }
                        icon.attr({
                            width: attr.width || 35,
    ...
    chatManageView.prototype = {
        ...
        updateChatTag: function(key, data) {
            ...
            //2016.04.022
            /*if(data.logo){
              $('.view-history-icon-div').css({'background':''});
              $('.view-history-icon-div').html(['<img data-single="1" onerror="nTalk.loadImageAbnormal(this, event)" class="view-history-receive-icon" userid="', data.userid, '" src="', data.logo, '" style="', $.STYLE_NBODY, 'width:35px; height:35px" />'].join());
            }*/
            return;
        },
        /**
           ...
           //2016.04.12 匹配图片还是颜色值
            ...
            var self = this,
                html = [
                    ...
                    '<div class="chat-view-window-header-title" style="', $.STYLE_BODY, 'color:'+titlename+';margin:0 85px;font-size:16px;font-weight:bold;text-align:center;line-height:'+header_title_height+'px;height:'+header_title_height+'px;overflow:hidden;"></div>',
            ...
            this.header = $({
                ...
                //2016.04.08更改高度
                style: $.STYLE_NBODY + title_whole_exp+';width:99.9%;height:'+header_title_height+'px;position:fixed;left:0;top:0;z-index:999;'
            ...
            var backButtonHtml = [
                "<div class='chat-view-window-header-back' style='position: absolute; width:30px; height:30px; z-index:50; left: 12px; top: 7px'>",
                //2016.04.7更改成为符号
            ...
            var closeButtonHtml = [
                "<div class='chat-view-window-header-ev-close' style='position: absolute; width:30px; height:30px; z-index:50;right: 12px; top: 7px;display:none;'>",
                //2016.04.7
            ...
            this.header.append(backButtonHtml).append(closeButtonHtml);
