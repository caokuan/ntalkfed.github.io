		...
		_addRightLabel: function(title, data, length, selected){
			...
			if( $.isArray(data) ){
				...
				for(var i = 0; i < data.length; i++){
					$({tag:'li', talk_title: data[i].title, talk_content: data[i].con, title: $.clearHtml(data[i].con || ''), style: $.STYLE_BODY + 'list-style:disc outside none;margin-top:5px;cursor:pointer;background-color:#FAF9F9;'}).appendTo(listElement).html( $.clearHtml(data[i].title || '') );
					/*
