        ...
        switchUI: function(type) {
            ...
            switch (type) {
                case this.mode.CON_VIEW_WINDOW:
                    ...
                    this.contains.find('.chat-view-window-header').css('height', '0px').display(1);
...
/*            dialogElement.find('table textarea').bind('keyup', function(event) {
            ...
            return type === 'right' ? [ //发送的消息
                    ...
                    (
                        ...
                        ['<div class="view-history-body" type="', data.type, '" style="', $.STYLE_BODY, 'line-height:160%;', (data.type == 2 ? 'color:'+bubble_text+';text-align:center;display:table-cell;*display:inline-block;vertical-align:middle;width:65px;height:65px;*font-size:0px;*line-heihgt:0px;*font-family:Arial;padding:2px;background:none;border-radius:5px;' : 'display:block;'), 'word-break:break-all;word-wrap:break-word;padding:6px;color:'+bubble_text+';background:'+bubble+';border:1px solid #a1cce8;border-color:'+bubble_border+';border-radius:5px;margin-left:50px;">', (/^(1|9)$/i.test(data.type) ? data.msg : ''),
                         ...
                         '<div class="view-history-pic-status" style="position: relative; top: -24px; margin:0px; left:-2px; font-size:12px;padding:0px; text-align:center; display:none"></div>'
                        ].join('')
                    ),
                    //==相关状态==
                    ...
                    '</svg>',
                    '</td>'].join("") :
                    ['<td valign="top" style="padding-right:5px;z-index:100;">',
                    '<svg height="10px" width="6px" id="svg" style="margin-top:13px">',
                    '<path d="M6,5 L0,0 L0,10 L6,5" style="stroke-width:1;fill:'+bubble+';stroke:'+bubble+';" />',
                    '</svg>',
                    '</td>'].join("")),
        ...
        _getViewHtml: function(type) {
            ...
            return type == 'load' ? [
                ...
                type === 'window' ? [
                    //分配客服时状态消息
                    '<div class="chat-view-window-status-info" style="', $.STYLE_BODY, 'overflow:hidden;background:#f9efda;width:100%;line-height:55px;height:55px;position:fixed;top:0px;display:none;z-index:300">',
                ...
                type == 'bottom' ? [
                    ...
                    '<div class="chat-view-window-editor" style="', $.STYLE_NBODY, 'position:relative;display:block;">', (
                        $.Audio && $.Audio.support() ? '<div class="chat-view-button-audio" style="' + $.STYLE_NBODY + 'width:32px;height:32px;position:absolute;top:5px;left:5px;cursor:pointer">' +
                        [
...
/*            var colorExp = /^#[0-9a-f]{6}$/i;
        ...
        _create: function() {
           ...
           //2016.04.12 匹配图片还是颜色值
            ...
            var self = this,
                html = [
                    '<div class="chat-view-window-header-message-title" style="', $.STYLE_BODY, 'display:none;color:#FFF;margin:0 85px;font-size:16px;font-weight:bold;text-align:center;line-height:55px;height:55px;overflow:hidden;"></div>',
                    '<div class="chat-view-window-header-title" style="', $.STYLE_BODY, 'display:none;color:#FFF;margin:0 85px;font-size:16px;font-weight:bold;text-align:center;line-height:55px;height:55px;overflow:hidden;"></div>',
            ...
            this.header = $({
                ...
                style: $.STYLE_NBODY + 'background:transparent;width:99.9%;height:0px;border-bottom:0px solid #5F6467;position:fixed;left:0;top:0;z-index:999;'
            ...
            var backButtonHtml = [
                "<div class='chat-view-window-header-back' style='position: absolute; width:30px; height:30px; z-index:50; border-radius:15px; left: 15px; top: 12px'>",
                //2016.04.7更改成为符号
                 '<div style="width:30px;height:30px;border-radius:50%;background-color:#FFF;opacity:0.7;">',
                 '<div style="'+return_close_style_css+'opacity:1;z-index:10;color:'+return_color+'">'+return_style+'</div></div></div>'].join("");
               /* '<svg width="32" height="32">',
            ...
            var closeButtonHtml = [
                "<div class='chat-view-window-header-ev-close' style='position: absolute; width:30px; height:30px; z-index:50; border-radius:15px; right: 15px; top: 12px;display:none;'>",
                //2016.04.7
                '<div style="width:30px;height:30px;border-radius:50%;background-color:#FFF;opacity:0.7;">',
                '<div style="'+return_close_style_css+'opacity:1;z-index:10;color:'+close_color+'">'+close_style+'</div></div></div>'].join("");
