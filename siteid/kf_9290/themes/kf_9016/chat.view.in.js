;(function($, undefined){
	...
	$.chatView.prototype = {
		...
		_create: function(){
			...
			//.chat-header-icon,.chat-header-name,.chat-header-sign,.ntalk-button-maxresize,.ntalk-button-min,.ntalk-button-close
			this._tempHeader    = this.options.chatHeader.find('.chat-header-icon,.chat-header-name,.chat-header-sign,.ntalk-button-min');
         ...
         //2017.07.10 火星时代网的定制
          _hxsd_special:function(){
            var hxsd_chat_view_window = this.contains.find('.chat-view-window');
          	var hxsd_chat_view_window_bottom=this.contains.find('.chat-view-window-bottom');
            var hxsd_chat_view_end_session=this.contains.find('.chat-view-end-session');
            var hxsd_chat_view_submit=this.contains.find('.chat-view-submit');
          	var hxsd_chat_view_options=this.contains.find('.chat-view-options');
          	var hxsd_chat_view_window_history=$('.chat-view-window-history');
          	var hxsd_chat_view_window_editor=$('.chat-view-window-editor').find('textarea');
            var hxsd_body_chat_containter=$('.body-chat-containter');
            var hxsd_chat_view_lbar=$('.chat-view-lbar');
            var hxsd_chat_view_options_menu=$('.chat-view-options-menu');
            var hxsd_view_fileupload_body=$('.view-fileupload-body');
            // var hxsd_view_history_body = $('.view-history-body');
			var hxsd_view_history_nowDestname = $('.view-history-nowDestname');
			var hxsd_ntalk_window_right = $('.ntalk-window-right');
			hxsd_ntalk_window_right.css({
				'width': '0px'
			})
			hxsd_chat_view_window.css({
				'position': 'relative'
			})
            // hxsd_view_fileupload_body.css({
            // 	'border-color':'#ffffff'
 
            // });
           hxsd_chat_view_window_editor.css({
            	height:$.browser.Quirks ? '85px' : '75px',
				'box-shadow':'white 0px 0px 0px inset',
				'background-color': '#fdfdfd'
              

            });
             hxsd_chat_view_window_history.css({
				'border-bottom':'1px solid #edebf5',
             	'width':'452px',
				 'height': '338px'
             });
            hxsd_body_chat_containter.css({
          	...
          	hxsd_chat_view_window_bottom.css('background','#fdfdfd');
          		
			hxsd_chat_view_end_session.css({
				margin:'4px 0',
        ...
        //    //发送按钮悬浮在上面
        //     hxsd_chat_view_submit.hover(function(event){
		// 		$.Event.fixEvent(event).stopPropagation();
		// 		$(this).css({
		// 			'background-color': '#f65f57',
		// 			'color': '#fff'
		// 		});
		// 		hxsd_chat_view_lbar.css('display','none');
		// 	}, function(event){
		// 		$.Event.fixEvent(event).stopPropagation();
		// 		$(this).css({
		// 			'background-color': '#f2f2f2',
		...
		showMessage: function(position, data){
			...
			if($(".welcome").length==1){
				$(".welcome").css("visibility","hidden").css("visibility","visible").css("margin-bottom","26px");
			}
		...
		_showFileUpload: function(liElement, bodyElement, data, type) {
			...
			var d_width = 270, d_height = [110, 80, 30], d_border = '1px solid #f0f0f0;', d_left = [13, 80], d_top = [10, -42];
			...
			//所需图标在大图中的位置
			//2016.03.30修改图片的位置
			var statusIconPosition = success ? ' -390px -220px ' : ' -394px -239px ';
			//显示状态名称
		...
		createEvaluation: function(formOptions, title, startColor, endColor, callback){
			var self = this,
				dialogElement,
				html = [
				'<div class="ntkf-alert-close" style="' + $.STYLE_NBODY + 'cursor:pointer;height:20px;position:absolute;right:5px;top:9px;width:20px;background:url(' + $.imageicon + ') no-repeat scroll -26px -73px;-moz-border-radius:0px;-webkit-border-radius:0px;border-radius:0px;"></div>',
		...
		_getMessageHtml: function(type, data){
			...
			if( type === 'otherinfo' ){
				...
				data.msg = [//faq信息
					'<h1 style="',$.STYLE_BODY,'">',
					'<span style="',$.STYLE_NBODY,'float:left;margin-right:5px;width:15px;height:15px;background:transparent url(',$.imageicon,') no-repeat -204px -63px;"></span>',
					...
					'<table style="',$.STYLE_NBODY,'float:right;_float:none;border-collapse:separate;" class="view-history-right" cellpadding="0" cellspacing="0" border="0" class="table">',
						'<tbody style="',$.STYLE_NBODY,'text-align:right;">',
							...
							//2016.03.11更改颜色更改border 顏色值 添加color 值if(data.type == 7) data.type = 1;
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#f65f57;border:1px solid #f65f57;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;">',
								...
								'<td style="',$.STYLE_NBODY,'width:10px;vertical-align:top;overflow:visible;">',
									...
									//尖角添加className
									'<div class="view-history-angle" style="',$.STYLE_NBODY,'position:relative;left:-1px;z-index:1;width:10px;height:18px;background:url(',$.imageicon,') no-repeat -68px -70px;"></div>',//
									
								'</td>',
								'<td style="',$.STYLE_NBODY,'width:54px;vertical-align:top;overflow:visible;">',
								//2016.03.31更改样式增加td//2016.03.18增加头像
								   '<div style="',$.STYLE_NBODY,'margin-right:14px;width:40px; height:40px; position:relative; bottom:0; background:url(',$.imageicon,') no-repeat -267px -38px; border-radius:50%;"></div>',
								'</td>',
							'</tr>',
							'<tr style="',$.STYLE_NBODY,'">',
								'<td style="',$.STYLE_BODY,'overflow:visible;text-align:right;position:relative;">',
								  '<span class="view-chat-hidden-area" style="',$.STYLE_NBODY,'width:1px;height:26px;overflow:visible;position:relative;top:0px;">',
										'<div class="view-history-status" style="',$.STYLE_BODY,'display:none;color:#010002;line-height:26px;width:280px;position:absolute;left:-280px;top:-17px;">',
											...
											'<div class="view-history-status-icon" style="',$.STYLE_NBODY,'margin:7px 3px;float:right;display:block;line-height:26px;width:10px;height:10px;background:#fff url(',$.imageicon,') no-repeat -151px -67px;"></div>',
										'</div>',
									'</span>',
								//2017.05.16 隐藏访客名称
									// '<span class="view-history-nowDestname" style="',$.STYLE_BODY,'color:#999999;line-height:26px;font-family:Arial;font-size:10px;">',
						...
						'<tbody style="',$.STYLE_NBODY,'">',
							'<tr style="',$.STYLE_NBODY,'">',
							    '<td style="',$.STYLE_NBODY,'width:48px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
									...
									'<div class="view-history-div" style="',$.STYLE_NBODY,'float:left;margin-left:8px; -margin-left:7px; width:40px; height:40px;'+(data.logo?' ':'background:url('+$.imageicon+') no-repeat -320px 0px;')+' display:inline-block; position:relative; top:0;border-radius:50%">',
							   ...
							   '<td style="',$.STYLE_NBODY,'width:10px;height:40px;vertical-align:bottom;vertical-align:top;overflow:visible;">',
									//2016.03.18更改垂直方向的位置
									//尖角添加className
									'<div class="view-history-angle" style="',$.STYLE_NBODY,'flaot:right;position:relative;top:0px;z-index:1;display:inline-block;width:10px;height:18px;background:url(',$.imageicon,') no-repeat -57px -70px;"></div>',
								'</td>',
								'<td class="view-history-content" style="',$.STYLE_BODY,'padding:8px;background:#f0f0f0;border:1px solid #f0f0f0;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px">',
									'<div class="view-history-body" style="min-height:20px;',$.STYLE_BODY,(/^(2|4)$/i.test(data.type)&&!data.emotion ? 'text-align:center;display:table-cell;*display:inline-block;vertical-align:middle;/*width:100px;*/min-height:50px;height:85px;*font-size:0px;*line-height:0px;*font-family:Arial;' : 'display:block;/*width:100%;*/'),'word-break:break-all;word-wrap:break-word;',(type=='bottom' ? 'width:60px;' : ''),
									(data.type == 1 ? 'color:#' + data.color + ';color:#666666;font:'+(data.italic=="true" ? 'italic' : 'normal')+' '+(data.bold=="true" ? 'bold' : 'normal')+' ' + data.fontsize + 'px/160%' + ' ' + 'Arial,SimSun;color:#4d4d4d;text-decoration:' + (data.underline=="true" ? 'underline' : 'none') + ';' : ''),
									'">',
			...
			return type=='load' ?
				...
				[
					...
					'<div class="chat-view-float-history" style="',$.STYLE_BODY,'width:100%;height:270px;height:267px\\0;_height:269px;background:#fff;padding-top:1px solid #fff\\0;position:absolute;overflow:hidden;z-index:99;display:none;box-shadow:0 5px 3px #888888;">',
						...
						'</iframe>',
					'</div>',
					'<div class="chat-view-window-history" style="',$.STYLE_BODY,'width:100%;height:338px;height:338px\\0;_height:269px;background-repeat:no-repeat;background-position:center bottom; padding-top:1px solid #fff\\0;position:relative;overflow-x:hidden;overflow-y:scroll;">',
					...
					'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
						'<div class="chat-view-window-status-info" style="',$.STYLE_BODY,'background:#f65f57;overflow:hidden;color:#fff;margin-left:10px;width:380px;line-height:30px;height:30px;position:absolute;top:-30px;z-index:99;text-align:center;display:none;"></div>',
					'</div>',
					'<span class="chat-view-xiaoneng-version" style="',$.STYLE_BODY,'display:block;position: absolute;top:316px;left:10px;visibility:visible;text-decoration:none;height:26px;line-height:26px;color:#999999;visibility:hidden;">\u5bf9\u65b9\u6b63\u5728\u8f93\u5165\u4fe1\u606f\u002e\u002e\u002e</span>',
					// //2016.03.11更改border 的颜色
					'<div class="chat-view-window-toolbar" style="',$.STYLE_BODY,'height:40px;width:280px;float:left;background:#fdfdfd;position: absolute;bottom: 0px;left:0px;">',
						...
						'<div class="chat-view-button chat-view-face" title=',$.lang.button_face,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 3px 20px;_margin-left:5px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat 0px 0px;padding:2px;">',
							'<div class="chat-view-hidden-area" style="',$.STYLE_NBODY,'width:0px;height:0px;position:relative;overflow:visible;">',
								'<div class="chat-view-span chat-view-window-face" style="',$.STYLE_NBODY,'display:none;position:absolute;left:-5px;top:-229px;border:1px solid #979A9E;width:273px;height:224px;background:#fff;z-index:1000002;cursor:auto;border-radius:3px;overflow:hidden;">',
									
								'</div>',
							'</div>',
						'</div>',
						//2016 更改图标padding 值
						'<div class="chat-view-button chat-view-image" title=',$.lang.button_image,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 20px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -40px 0px;padding:2px;"></div>',
						'<div class="chat-view-button chat-view-file" title=',$.lang.button_file,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 20px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -83px 0px;padding:2px;"></div>',
						//2016.03.21更改位置
						'<div class="chat-view-button chat-view-capture" title=',$.lang.button_captureImage,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 20px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -121px 0px;padding:2px;"></div>',
						/*'<div class="chat-view-capture-options" style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 0px;border:0px solid #ccc;height:16px;display:inline-block;cursor:pointer;">',
							...
							'</div>',
						'</div>',*/
						'<div class="chat-view-button chat-view-evaluate" title=',$.lang.button_evaluation,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 20px;border:0px solid #ccc;height:18px;display:inline-block;cursor:pointer;width:15px;background:url(',$.imageicon,') no-repeat -158px 0px;padding:4px 5px 0px 4px;"></div>',
						'<div class="chat-view-button chat-view-history" title=',$.lang.button_save,' style="',$.STYLE_BODY,'color:#525252;float:left;margin:8px 0 4px 20px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:20px;background:url(',$.imageicon,') no-repeat -200px 2px;padding:2px;"></div>',
						...
						'<div class="chat-view-switch-manual chat-view-robot-button" title="',$.lang.button_switch_manual,'" style="',$.STYLE_BODY,'color:#525252;float:left;padding:0 0 0 20px;margin:10px 0 10px 6px;border:0px solid #ccc;height:20px;display:inline-block;cursor:pointer;width:auto;background:url(',$.imageicon,') no-repeat -405px -129px;display:none;">',$.lang.button_switch_manual,'</div>',
					...
					'<div class="chat-view-window-editor" style="',$.STYLE_BODY,'height:90px;width:100%;overflow:hidden;">',
						//2016.03.22更改固定文字 更改颜色
						'<textarea placeholder="\u6211\u6709\u95ee\u9898\uff0c\u6211\u8981\u95ee\u002e\u002e\u002e" style="',$.STYLE_BODY,CON_STYLE_SHADOW,'margin:1px;padding:15px 0 0 20px;width:391px;width:411px\\9;height:73px;height:93px\\9;outline:0px solid #08f;border:0px solid #08f;color:#999999;font-family:Arial;resize:none;overflow-x:hidden;overflow-y:auto;resize: none;"></textarea>',
					'</div>',
					...
					/*	'<div class="chat-view-options" style="',$.STYLE_BODY,'margin:6px 10px 6px 0;float:right;border:1px solid #ccc;width:14px;height:26px;line-height:25px;text-align:center;cursor:pointer;">',
							...
							'</div>',
						'</div>',*/
						'<div class="chat-view-submit" style="',$.STYLE_BODY,'margin:5px 0;float:right;width:65px;height:30px;line-height:30px;text-align:center;padding:0;border:1px #CCC solid;border-right:none;cursor:pointer;">'
						//2016.03.22更改固定字体
						,'\u53d1\u9001\u0028\u0053\u0029',
						'</div>',
		...
		_bind: function(){
			...
			this.textEditor = this.chatElement.find('.chat-view-window-editor textarea').css({
				...
				$(this).attr('placeholder','\u6211\u6709\u95ee\u9898\uff0c\u6211\u8981\u95ee\u002e\u002e\u002e');
				self._stopListen();
			...
			this.chatElement.find('.chat-view-evaluate').click(function(event){
				...
				if( $(this).attr('talk_disable') ) return;
				//2017.05.15 点击后样式
				// $(this).css("background-position","-158px -25px");
				...
				$(this).css("background-position","-158px 0px");
			})
		...
		callChatResize: function(width, height){
			...
			this.chatHistory.css({'height': (height - 131) + 'px'});
			this.chatElement.find('.chat-view-float-history, .chat-view-float-history iframe').css({'height': (height - 131) + 'px'});
	...
	$.chatManageView.prototype = {
		...
		updateChatTag: function(settingid, data, updateStatus){
			...
			this.leftContent.find('li.'+settingid+' .tag-head-icon').css('background-color', data.status!==1 ? '#666' : '#f65f57');
			...
			var nameMarginTop = data.sign? '10px': '17px';
			//2016.03.21 修改name的值增加在线客服的字样
			name.css({'visibility':'visible','margin-top':nameMarginTop}).html( '\u5728\u7ebf\u5ba2\u670d\u002d'+data.title || '\u5728\u7ebf\u5ba2\u670d\u002d'+data.name );
			signWidth = Math.max(0, this.header.width() - name.width() - 95);
			sign.css('visibility', 'visible').attr('title', data.sign).html( data.sign );
		},
		/**
		 * 切换标签
		 * @param  {String} settingid
		...
		toggleExpansion: function(attr, extend){
			...
			this.headNS.css('width',this.chatWidth - 104 - 140 + 'px');
		...
		_create: function(){
			...
			this.btnHover = $({tag:'div',title:'展开',className:'nTalk-btnHover chat-view-exp',style:'width:10px;height:20px;position:absolute;top:61px;left:445px;z-index:99999;display:block;cursor:pointer;background-color: #dadada;border-radius:2px;'}).appendTo($('.ntalk-window-containter'));
			this.btnHover_arrow=$({tag:'a',className:'nTalk-btnHover-arrow',style:'display:block;width:10px;height:20px;position:absolute;top:0px;left:0px;background:url('+$.themesURI+'chaticon.png) no-repeat -292px -222px;z-index:1000002;background-color:transparent;'}).appendTo(this.btnHover);

			this._objView.buttonClose.css({
				margin: '0',
				width:'30px',
				height: '50px',
				display: 'none',
				background: 'url(' + $.imageicon + ') no-repeat -21px -58px'
			});
			if( this._objView.buttonResize ){
				this._objView.buttonResize.css({
					'width':  '12px',
					'height': '15px',
					'background': 'url(' + $.imageicon + ') no-repeat -287px -5px'
				});
			}
			this._objView.buttonMax.css({
				'display': 'none'
			}).attr('title', $.lang.chat_button_resize_max);
            //2016.03.15更改
			this._objView.buttonMin.css({
				margin: '0px 0px 0 0',
				width:'60px',
				height: '50px',
				background: 'url(' + $.imageicon + ') no-repeat 4px -52px'
			});
		
			this.headBody = $({className: 'chat-header-body', style: $.STYLE_BODY + 'z-index:0;color:#525252;'}).appendTo(this.header, true).css({
				'position': 'absolute',
				'border-left': '1px solid #f65f57',
				'border-right': '1px solid #f65f57',
				'border-bottom': '0',
				'top': '0px',//IN
				'border-right': '1px solid #f65f57',//IN
			...
			this.headPic=$({tag: 'span', className:'chat-header-pic', style: $.STYLE_BODY + 'display:inline-block;margin:15px 22px 0 20px;height:20px;width:120px;float:left;background:url('+ $.imageicon + ') no-repeat -305px -78px' }).appendTo(this.headBody).html('');
			//2016.03.11更改字体颜色和margin 值
			this.headSolid=$({tag: 'span',className:'chat-header-solid',style:'display:inline-block;height: 15px;width:1px;background-color: #fff;float:left;margin:18px 15px 0 0;'}).appendTo(this.headBody).html('');
			this.headNS = $({tag: 'span',className:'chat-header-ns',style:'display:inline-block;height:50px;width:'+ (this.options.width - 246) + 'px;visibility:hidden;overflow:hidden;cursor:auto;'}).appendTo(this.headBody);
			this.headName = $({tag: 'span', className:'chat-header-name', style: $.STYLE_BODY + 'color:#ebedec;padding:0;margin:10px 0px 0px 0px;display:inline-block;heigth:13px;color:#ffffff;font-size:12px;float:left;max-width:220px;visibility:hidden;overflow:hidden;cursor:auto;'}).appendTo(this.headNS);
			
			this.headSign = $({tag: 'span', className:'chat-header-sign', style: $.STYLE_BODY + 'color:#c3c3c3;margin:0px;display:inline-block;height:20px;width:100%;visibility:visible;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;cursor:auto;'}).appendTo(this.headNS);
			//2016.03.10将图片进行隐藏
			...
			var pptElement_tem_height=270;
			this.pptElement         = $({tag: 'iframe', className: 'window-right-pro-images', style: $.STYLE_BODY + 'margin:0px;padding:10px 10px 15px 10px;width:' + (this.options.rightElementWidth- 21) + 'px;background:#fff;height:'+ pptElement_tem_height+'px;overflow-x:hidden; scroll-x:none'}).attr({
				//debug temp
				//别人提供的地址
				"src": "http://m.hxsd.tv/activity/market"
		...
		_bind: function(){
			...
			this.btnHover.hover(function(){
				...
				if($(this).attr('title') == '展开') {
					...
					self.toggleExpansion('rightElement',true);
					$(this).attr('title','收起').css({
						'left':'455px',
						'display':'block'
					});
					$('.nTalk-btnHover-arrow').css({'background-position':'-304px -222px'});
					$.Event.fixEvent(event).stopPropagation();
					//2016.03.30修改
					//self.chatElement.find('.chat-view-window-face').display();
				} else {
					//self.toggleRightLabel(1);
					self.toggleExpansion('rightElement',false);
					$(this).attr('title','展开').css({
						'left':'445px',
						'display':'block'
					});
					$('.nTalk-btnHover-arrow').css({'background-position': '-292px -222px'});
					$.Event.fixEvent(event).stopPropagation();
					//2016.03.30修改
					//self.chatElement.find('.chat-view-window-face').display();
				}
		...
		_onOverChatTag: function(event){
			...
			$(target).find('.tag-button-close').css({
				background: 'url('+$.imageicon+') no-repeat -150px -67px'
			});
		...
		_callResize: function(args){
			...
			var rigthConentHeight = this.options.height - 38 - 300 - 40;
			//2016.03.21新添加
		...
		_addRightLabel: function(title, data, length, selected){
			var self = this,
				...
				//2016.03.21更改高度
				rightContentHeight = this.options.height -338 - 40;
			if( !this.rightTags ){
				//2016.03.11更改颜色 border 改为1
				this.rightTags = $({className:'window-right-tags', style: $.STYLE_NBODY + 'background:#ffffff;z-index:-1;overflow:hidden;height:37px;'}).appendTo(this.rightBody);
				this.rightTags.insert('<div style="' + $.STYLE_NBODY + 'clear:both;"></div>');
			}
			if( !this.rightContent){
				//2016.03.11
				this.rightContent = $({className:'window-right-content', style: $.STYLE_NBODY + 'overflow:hidden;border-top: 1px solid #f4f2fc;background:#fafafa;position:relative;top:-1px;z-index:1;border-radius:0px 0px 5px 0px;-moz-border-radius:0px 0px 5px 0px;-webkit-border-radius:0px 0px 5px 0px'}).appendTo(this.rightBody);
			}
			//2016.03.11改变字体border颜色 
			style = $.STYLE_BODY + 'background-color:#ffffff;height:36px;color:#cccccc;line-height:36px;text-align:center;cursor:pointer;float:left;';
			...
			if( length == 1 ){
				...
				if( this.rightTags.find('div').length == 1 ){
					...
					style += 'width:' + (length == 2 ? 98 : 64) + 'px;';
				}else if( this.rightTags.find('div').length < length ){
					style += 'width:' + (length == 2 ? 98 : 64) + 'px;';//border-left:1px solid #FCE4E7;
				}else{
					//2016.03.29 修改大小
			...
			tagElement = $({className: key, title: title, style: style}).appendTo(this.rightTags, this.rightTags.find('div').last()).html( '<span style="width: 56px; height: 34px; display: block; margin: 0 auto;font-size: 14px;"> '+ title +'</span>' );//.gradient("top", '#F8CEDC', '#FAB2CA')
			//2016.03.21更改颜色 增加padding 值width值
			tagBody = this.rightContent.insert( ['<div class="',key,' view-right-content" style="',$.STYLE_BODY,'background-color:#fafafa;width:'+ (this.options.rightElementWidth - 40) +'px;height:' + rightContentHeight + 'px;overflow-x:hidden;padding:20px;;overflow-y:auto;display:none;border-radius:0px 0px 5px 0px;-moz-border-radius:0px 0px 5px 0px;-webkit-border-radius:0px 0px 5px 0px"></div>'].join('') );
			
			if( $.isArray(data) ){
				//2016.03.11修改list样式和颜色值
				//用于常见问题一类的列表形式展示内容
				listElement = $({tag: 'ul', style: $.STYLE_BODY + 'margin:0px 0px 4px 0px;list-style:none;background-color:#fafafa;color:#999999;'}).appendTo(tagBody).click(function(event){
					var srcElement = $.Event.fixEvent(event).target;
				...
				$({className: 'window-right-text', style: $.STYLE_BODY + 'margin:5px;color:#999999;font-size:10px;'}).appendTo(tagBody).html(data);
			}
			//新创建标签默认选中
			if( selected ){
		...
		_selectedTag: function(eventElement){
			...
			// 		//2016.03.11改变背景颜色高度值
			// 		$(elem).addClass('talk_selected').css({'height': '36px', 'border-bottom': '1px solid #f65f57'});//.gradient('top', '#F8BAC2', '#F9CDDC').css('color', '#DE7E80');
			// 		self.rightContent.find('.'+key).display(1);
			// 	}else{
			// 		//$.Log('selected class:' + key + '');
			// 		$(elem).removeClass('talk_selected').css({'height': '36px', 'border-bottom': 'none'});//.gradient('top', '#F8CEDC', '#FAB2CA').css('color', '#FFFFFF');
			...
			this.rightTags.find('div[class]').each(function(i, elem){
				...
				if( $(eventElement).indexOfClass( key ) ){
					//$.Log('selected class:' + key + ', selected');
					$(elem).addClass('talk_selected').css({'height': '32px', 'border-bottom': 'none', 'color': '#666666'});//.gradient('top', '#F8BAC2', '#F9CDDC').css('color', '#DE7E80');
					$(elem).find('span').css('border-bottom', '1px solid #f65f57');
					self.rightContent.find('.'+key).display(1);
				}else{
					//$.Log('selected class:' + key + '');
					$(elem).removeClass('talk_selected').css({'height': '32px', 'border-bottom': 'none','color': '#cccccc'});//.gradient('top', '#F8CEDC', '#FAB2CA').css('color', '#FFFFFF');
