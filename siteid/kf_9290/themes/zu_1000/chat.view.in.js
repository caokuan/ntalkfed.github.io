		...
		_getMessageHtml: function(type, data){
			...
			if( type === 'otherinfo' ){
				...
				data.msg = [//faq信息
					...
					// '</h1>',
					'<p style="',$.STYLE_BODY,'color:#fff;">', data.title, '</p>'
				].join('');
			}
	...
	$.chatManageView.prototype = {
		...
		_create: function(){
			...
			this.pptElement         = $({tag: 'iframe', className: 'window-right-pro-images', style: $.STYLE_BODY + 'margin:0px;width:' + (this.options.rightElementWidth) + 'px;background:#fff;height:'+ pptElement_tem_height+'px;overflow-x:hidden; scroll-x:none'}).attr({
				...
				'src': 'http://192.168.31.49/test/lunbo.html'
			...
			if( $.isArray(data) ){
				...
				if(data.indexOf('<br>') > -1) {
					...
					for(var i = 0; i < data.length; i++){
						...
						$({className:'activeList',tag:'li', talk_title: data[i], talk_content: data[i],  style: $.STYLE_BODY + 'list-style:none outside none;padding:5px 10px;padding-right:none;cursor:pointer;background-color:#ffffff;color:#666666'}).hover(function(event) {
								$.Event.fixEvent(event).stopPropagation();
								$(this).css({
									'list-style':'none outside none',
									'cursor':'pointer',
							...
							}).appendTo(listElement).html( $.clearHtml(data[i] || '') );
					}
				}
				
			}else if(title.indexOf('\u5546\u54C1\u8BE6\u60C5') > -1) {
				//http://nt-v1-mct6.ntalker.com//goodsinfo/api.php?siteid=kf_4612&itemid=meilan_metal&itemparam=&sellerid=&user_id=&type=jsonp&lan=zh_cn&callback=scriptCallReceiveGoodsinfo_kf_4612_1492159739240&ts=1496393911976
				listElement = $({tag: 'ul', style: $.STYLE_BODY + 'margin:0px 0px 4px 0px;list-style:none;background-color:#ffffff;'}).appendTo(tagBody).click(function(event){
					var srcElement = $.Event.fixEvent(event).target;
					if( srcElement.tagName.toLowerCase() !== 'li' ) return;
				});
				...
				// queryString = $.toURI({
				// 	    "item_number":NTKF_PARAM.itemid,
				// 	    'token': '1f2c06954b0270c1a9eb2e16195426fd',
				// 		'callback': 'callback_'+ $.randomChar(),
				// 		'_': $.getTime()
				// 	});
				// var options = {
				// 	url: "http://detail.meizu.com:10082/api/item/xn"+ '?' + queryString,
				// 	type: 'GET',
				// 	data: {},
				...
				getData = function(){
					var callback = 'scriptCallReceiveGoodsinfo_' + NTKF_PARAM.settingid;
					var queryString = $.toURI({
							'itemid':NTKF_PARAM.itemid,
							'siteid':NTKF_PARAM.siteid,
							'itemparam':NTKF_PARAM.itemparam || '',
							'user_id': NTKF_PARAM.user_id || '',
							'type': 'jsonp',
							'lan': NTKF.language || 'zh_cn',
							'callback': callback,
							'ts': $.getTime()
						});
					var url = $.flashserver.mcenter + '/goodsinfo/api.php';
					console.log(url);
					window[callback] = function(result){
						...
						// }
						console.log(result)
						$({className:'parameterList',tag:'li', style: $.STYLE_BODY + 'list-style:none outside none;padding:11px 0px 5px 0px;padding-right:none;background-color:#ffffff;color:#666666;'}).appendTo(listElement).html(
							'<img src="'+result.item.imageurl+'" width="76px"'+ 'style="width=76px;display:inline-block;">');
						$({className:'parameterList',tag:'li', style: $.STYLE_BODY + 'list-style:none outside none;padding:5px 10px;padding-right:none;background-color:#ffffff;color:#666666;height:30px;line-height: 30px;'}).appendTo(listElement).html(
							'<span style="'+$.STYLE_BODY+'display:inline-block;width: 70px;color: #999999;">'+'商品名称'+'</span> <sapn style="'+$.STYLE_BODY+'display:inline-block;color: #666666;">'+(result.item.name || '')+'</span>');
						$({className:'parameterList',tag:'li', style: $.STYLE_BODY + 'list-style:none outside none;padding:5px 10px;padding-right:none;background-color:#ffffff;color:#666666height:30px;line-height: 30px;'}).appendTo(listElement).html(
							'<span style="'+$.STYLE_BODY+'display:inline-block;width: 70px;color: #999999;">'+'折后销售价'+'</span> <sapn style="'+$.STYLE_BODY+'display:inline-block;color: #666666;">'+(result.item.siteprice || '')+'</span>');
						$({className:'parameterList',tag:'li', style: $.STYLE_BODY + 'list-style:none outside none;padding:5px 10px;padding-right:none;background-color:#ffffff;color:#666666;height:30px;line-height: 30px;'}).appendTo(listElement).html(
							'<span style="'+$.STYLE_BODY+'display:inline-block;width: 70px;color: #999999;">'+'商品销售价'+'</span> <sapn style="'+$.STYLE_BODY+'display:inline-block;color: #666666;">'+(result.item.marketprice || '')+'</span>');
						$({className:'parameterList',tag:'li', style: $.STYLE_BODY + 'list-style:none outside none;padding:5px 10px;padding-right:none;background-color:#ffffff;color:#666666;height:30px;line-height: 30px;'}).appendTo(listElement).html(
							'<span style="'+$.STYLE_BODY+'display:inline-block;width: 70px;color: #999999;">'+result.item.custom1[0]+'</span> <sapn style="'+$.STYLE_BODY+'display:inline-block;color: #666666;">'+(result.item.custom1[1] || '')+'</span>');
						//var str = arr.join("");
						//console.log(str);
